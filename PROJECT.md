...
- **自宅外アクセス**: Tailscale 導入済 (2026-05 以前完了。`miho-inspiron-3521` / `m-gamepc` / `pixel-6` が tailnet 同居)。Tailscale 起点の単発ユーティリティとしてスマホ専用リモコン PWA (`~/companion/remote/`) を 2026-05-20 初期化 (フェーズ外、ロードマップ独立、設計議論起動準備済)。**境界原則** (M-14、軸 5 集約): remote/ PWA から bot.service / voice/ engine を直接叩く経路は採用しない (Discord 経由 + OWNER_ID 認可で統一)。remote/ PWA は Discord webhook / bot.service Unix socket への OWNER 認証経路設計を `~/companion/workspace/redesign/remote-design.md` で確定する前に bot / voice / maintenance のいずれも触らない (voice-design.md §0 dashboard コード変更ゼロ死守原則と整合)
