...
- **新規プロジェクト案: スマホで遊べるゲームをぜんぶ AI で作る** (2026-06-02 追加, someday): スマホで遊べるゲームを全部 AI (claude) で作る構想。ロードマップ Phase 外の独立プロジェクト候補。スコープ (ジャンル / 本数 / プラットフォーム = PWA か native か / 配布方法) も着手可否も未定。種段階。関連: vault `notes/2026-05-06_cli-game-candidates.md` (CLI ゲーム候補)、vault `aidiary/2026-04-11_games-i-want-to-try.md`
