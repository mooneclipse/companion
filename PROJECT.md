...
│   └── .claude/  # skills: orc / newgame、agents: code-reviewer / implementer / game-designer / game-critic / playtester / deep-worker（複数ファイル横断の実装丸投げ用、親モデル継承）/ researcher（読み取り専用調査用、sonnet。独立調査は並列分割）。委譲ルーティング規則は skills/orc/SKILL.md
├── logs/         # bot 動作ログ
└── (将来追加)
...
- **(C) ローカル git のみ（remote なし、rollback 専用）**: 複数セッション/agent team で大幅改稿される設計台帳など、「やり直し」が効けば十分でマシン外バックアップは不要なもの。`workspace/` `web/` `remote/` `bot-workspace/` `games/` `ytcheck/` が該当 (bot-workspace は 2026-06-10 Step 1 閲覧自由化に伴い追加 — settings.json が bot 経路 permission 境界の要で rollback 価値が高いため。ytcheck は 2026-07-07 移行時に OWNER 指定「新規 private repo になるならローカル git」で追加 — Windows 側原本削除後は USB バックアップが唯一のマシン外コピーになる点は承知の上)。下記「(C) の手順」。**GitHub に上げ忘れているのではなく意図的に remote なし**。誤って remote を足さないこと。
