...
// ⑥-b 修正必須(A): surfaceReturn の「帰れず地中残留」経路で companion が未解除のまま残らないこと。
// startDive 直後は G.dug が空 = 地中の女の子から自機(地表)への通り道が無いので bfsStep は即 null を返し、
// 帰還ループは break する(girl は following・row>0 のまま地中残留)。修正前はこのとき G.companion がその
// 残留 girl を指したまま全回復→継続し、次の潜行で effCompanionAtk が乗り killMonster で cexp が再加算される
// 「地表で別れる前に援護が乗り続ける」抜け道だった。修正で surfaceReturn 後に G.companion===null(清算なし解除)。
const stranded = await page.evaluate(() => {
  startDive();
  const g = G.girls[0];
  g.state = "following";
  g.col = 0; g.row = 5; // 地中。G.dug が空なので地表(自機)への通り道は存在しない。
  g.level = 4; g.cexp = 25; // 援護源・端数を持たせておき、解除で破棄(清算しない)を確認する。
  setCompanion(g);
  const companionBeforeReturn = (G.companion === g);

  surfaceReturn(); // 帰還ループは bfsStep=null で break、girl は地中残留のまま。

  const stillFollowing = (g.state === "following"); // 地表に上がれていない=following 継続。
  const stillUnderground = (g.row > 0); // 地中に残っている。
  const companionDetached = (G.companion === null); // 修正で解除済み(これが要)。
  const levelUnchanged = (g.level === 4); // 清算していない=level は繰り上がらない。
  const cexpUnchanged = (g.cexp === 25); // 清算していない=端数は cexp に残る(破棄=次回持ち越し先が無い)。

  // 抜け道検証: 解除済みなのでこの残留 girl に援護も cexp 再加算も乗らない。
  const atkAfterDetach = effCompanionAtk(); // companion=null → +0。
  const foe = { key: "SNAKE", col: 1, row: 1, hp: 0 }; // SNAKE exp=6。
  G.monsters = [foe];
  const cexpBeforeKill = g.cexp;
  killMonster(foe);
  const cexpNotReadded = (g.cexp === cexpBeforeKill); // companion 解除済み→再加算されない。

  return {
    companionBeforeReturn, stillFollowing, stillUnderground, companionDetached,
    levelUnchanged, cexpUnchanged, atkAfterDetach, cexpNotReadded,
  };
});
check("修正必須A=帰れず地中残留で companion 解除(清算せず)・残留 girl に援護/cexp 再加算なし",
  stranded.companionBeforeReturn && stranded.stillFollowing && stranded.stillUnderground &&
  stranded.companionDetached && stranded.levelUnchanged && stranded.cexpUnchanged &&
  stranded.atkAfterDetach === 0 && stranded.cexpNotReadded, stranded);
