...
await ctx.addInitScript(() => { try { localStorage.setItem("mineroad_seen_howto", "1"); localStorage.removeItem("mineroad_save"); } catch (e) {} });
...
const followA = await page.evaluate((driver) => {
  eval(driver);
  try { localStorage.removeItem("mineroad_save"); } catch (e) {} startDive();
