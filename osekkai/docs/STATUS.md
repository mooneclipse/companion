...
- [ ] 7. bot 側: proactive-v1 envelope の kind 追加 + osekkai topic の on_message 分岐 (専用 system prompt、短 timeout 個別指定、手動開始時の号令済みマーク) — **反映後は改変規模に応じた短い様子見 (PROJECT.md 条件 #2 再定義ルール)**。**前提条件 (必読、レビュー指摘)**: `companion-osekkai-{call,retro}.timer` の enable は **本 TODO 7 実装完了後に限る**。現行 bot.py は `kind != "proactive"` の envelope を `parse_proactive_payload` (bot.py:707) で `None` にした後、素通し forward 経路 (bot.py:3407 `notify_queue` → `_notify_worker` → `send_text`) にフォールバックさせる。つまり TODO 7 未実装のまま timer を有効化すると、「弾かれて無害」ではなく **envelope の生 JSON (backlog の自由文・tonight.intent を含む) がそのまま maintenance thread に届いてしまう**。**申し送り (TODO 6)**: (1) `parse_proactive_payload` の `kind != "proactive"` ゲートを緩め、`kind == "osekkai"` は **`proactive_queue` とは別の専用ディスパッチ** (mode: "call"/"retro" で分岐) にすること — 既存の `build_proactive_prompt` はフィールドをサニタイズ済み whitelist でしか展開しない設計 (bot.py:765-773) なので、そこに osekkai envelope を通すと `backlog` / `tonight` が黙って落ちる。(2) `backlog[].text` / `tonight.intent` は **OWNER 自身が書いた自由文** であり、proactive 経路の「サニタイズ済みフィールドのみ展開」規約とは前提が異なる (OWNER 入力なので通常のチャット入力と同格に扱ってよい)。(3) `_handle_notify_connection` (bot.py:3397) は単発 `reader.read(NOTIFY_SOCKET_MAX_BYTES)` — `asyncio.StreamReader.read(n)` は「バッファに溜まっている分があればそれだけで即返す」仕様で EOF までのループ読みではないため、envelope が大きい (backlog 大量時など、送信側の 1 回の `sendall()` がカーネル受信バッファに収まりきらない場合) と理論上部分読みが起きる。今回の検証サイズ (数百バイト) では問題は表面化しなかったが、TODO 7 でループ read への変更かサイズ上限の明示を検討すること。(4) envelope 実例・スキーマは下記 TODO 6 Done エントリ参照
...
**最終更新**: 2026-07-16 (TODO 6 trigger 完了、レビュー指摘 3 点反映済み、commit a05cfa6。次は TODO 7 bot 分岐)
