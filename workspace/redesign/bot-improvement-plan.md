...
## Step 2: bot.py 小改変パック #1 — 画像応答 + permission_denials 記録 (B-3 締め 6/11 後) — ✅ 完了 2026-06-10 (実弾検証 2 件 pass)

実装記録は `bot/docs/STATUS.md` Done「Step 2 bot.py 小改変パック #1 (C-2 完了)」参照。B-3 観察締めは 13 日分全項目クリーンを根拠に OWNER 承認で 1 日前倒しクローズ (同 Done「Telegram 観察締め (B-3 完了)」)、ゲート解除して同日着手。commits: 0dcd926 (2-1) / a28772a (2-2) / f0b648a (review 反映)。151 tests pass + restart 後の実弾 2 件 (キャプションあり/なし) pass。2-2 は bot.py でなく quota.py `_record_common` (ledger 書き込み単一集約点) に実装。残りは数日の NRestarts/ERROR 様子見のみ。
...
> **2026-06-12 改訂 (ticket #16 議論)**: クレジット枠は暦月でなくサブスクリプション課金日 (月の途中) 起点で更新されるため、「月初起点の集計 + 月末着地予測」を「課金窓起点」に引き直した (3-0 新設、3-1/3-2 を窓基準に修正)。また `claude -p "/usage"` が headless でサーバ側プラン利用率を $0 / API 消費ゼロで返すことを実機確認 (2026-06-12、319ms / num_turns 0) し、公式値の併記 (3-4) を追加。個人アカウントでは Usage & Cost Admin API は使えない (公式 doc に individual accounts 不可と明記) ため、機械取得できる公式値はこれが唯一。

### 3-0. 集計窓を課金アンカー日起点に変更 (3-1/3-2 の前提)

- `quota.py` の「本月」(暦月 1 日起点) を「課金窓」(アンカー日起点、JST) に変更する。対象は `CreditBudgetGuard.allow()` / `summary()` の `cost_month` / `format_summary` の表示 / 3-1 の警告判定で、**全箇所が同じ窓関数を使う** (窓定義を二重化しない)。`exceeded_message` の「月初までクールダウン」と CreditBudgetGuard docstring の「月初から累計」の文言も窓基準に直す
- アンカー日は env `BOT_CREDIT_ANCHOR_DAY` (例: `15`)。**正確な課金更新日は OWNER がアカウント画面/メールで確認してから設定する** (2026-06-12 時点では 15 と仮置き)
- 移行初月 (6/15 制度開始) は「ledger は 6/1 から記録、クレジット窓は 6/15 開始」のズレがあり、窓起点に変えるとガードが正しく 6/15 起点で数える (従来の暦月集計は安全側に多めに数えていた)

### 3-1. ソフト警告 (50% / 80%)

- `record()` 後に課金窓内累計が 50% / 80% を跨いだら #maintenance topic に 1 回だけ通知 (例: `[budget] 窓内累計 $50.12 / $100 (50% 到達)`)
- 「窓 × 閾値ごとに 1 回」の発火管理は state ファイル (`sessions/budget-notified.json` 等) を 1 回引いて確定 (2 周目ルール整合: 文言マッチ・リトライなし)
- 現状はハード上限到達時のみ表面化するため、その手前の計器を足すのが目的

### 3-2. /quota に窓終端の着地予測

`format_summary` に 1 行追加: `着地予測: $XX.XX (現在ペース × 窓残日数)`。課金窓開始からの日割りペースを窓終端 (次のアンカー日) へ単純外挿する (精密予測は過剰)。暦月の月初日割りは課金窓とずれて参考にならないため不採用 (OWNER 指摘 2026-06-12)。
...
### 3-4. /quota に公式プラン利用率を併記

- `/quota` 実行時に `claude -p "/usage"` を subprocess で 1 回叩き、サーバ側の公式利用率 (5h セッション窓 / 週次窓の % と reset 時刻) を自前 ledger 集計の下に併記する。実測: $0 / num_turns 0 / 319ms (2026-06-12)
- 自前 ledger = bot 経由分のみ / 公式値 = アカウント全体 (手元セッション込み)、という違いを表示文言で明示する
- 出力は人間可読テキストで書式が CLI バージョンで変わり得る。**parse せず全文をそのまま転記**する (文言マッチ分岐をしない = 2 周目ルール整合)。取得失敗時は「公式値取得失敗」の 1 行のみで本体集計は出す (best-effort)
- `~/companion/CLAUDE.md` の「claude CLI バージョン up 時の再検証」リストに `/usage` headless を追加済み (2026-06-12)

### 6/15 制度開始後の再検証 (実装前提の確認項目)

- `claude -p "/usage"` の出力にクレジット枠 ($100) の行が増えるか → 増えれば 3-4 がそのままクレジット残量の公式表示になる
- `/usage-credits` コマンド (Pro/Max の usage credits 月次支出上限、公式 doc 記載) の実機挙動 → Anthropic 側ネイティブ上限として CreditBudgetGuard の補完になり得る
- Console Usage ページでの claude -p 消費の見え方 → K-14 (7/1 前後の Console vs ledger 突き合わせ) と合流

### 完了条件

各項目 1 commit + テスト + restart + 実弾 (/quota /status 表示確認)。50%/80% 通知は実弾困難なため state 注入のユニットテストで代替。3-0 の窓関数はアンカー日跨ぎ (窓開始日当日 / 前日) の境界をユニットテストで確認 (アンカー日が 29〜31 と確定した場合は 2 月等の短い月のクランプ仕様を 3-0 に追記してからテスト)。
