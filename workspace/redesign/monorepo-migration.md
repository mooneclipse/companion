...
**status**: Phase 0〜4 + OWNER 3 コマンド完了 (2026-07-11、チケット #82 done) — 初回 push 済み (main = origin/main = f3b883a 実測)、旧 4 repo isArchived=true ×4 実測、.migration 削除済み。Phase 5 検証 (§8) は vault auto-sync / Task Workflow / ytcheck 共存 / ytcheck 自動 commit 実弾 (2026-07-15、45a7c39) まで確認済み (チケット #85)。残り = 2〜4 週後の .archive/split-era-git 削除のみ (OWNER 承認後、7 月末目安)
...
- 事後トラブル 1 件 (2026-07-11 朝、解決済み): 旧 4 repo アーカイブの際に `mooneclipse/companion` 本体も誤って archive され、以後の push が read-only で reject。`gh repo unarchive mooneclipse/companion --yes` で解除し再 push で同期 (旧 4 repo の isArchived=true は維持を確認)。GitHub 側に archive→unarchive のイベントが残るのは正常
...
- [x] 翌朝 05:00 の ytcheck run とモノレポの共存 (2026-07-11 実測): 05:03〜05:19 正常完了 (推薦 11 件)、モノレポは run 後も clean = 巻き込みなし。**commit ゼロが正常** — `_git_commit` はチャンネル追加/更新 (`add_channel`/`update_channel`) 時のみ呼ばれ、日次巡回は vault (境界外) と gitignore 済み cache しか書かない。§3-1 の「05:00 回避」と本チェック項目は「日次で自動 commit が載る」前提だったが実装はそうではなかった (害なし)。自動 commit の実弾は次にチャンネル追加/更新が起きた際に `git log -- ytcheck/tasks/` で確認 → **2026-07-15 検証 OK (チケット #85)**: 同日 05:04 に `45a7c39 tasks: 登録者数キャッシュ自動書き戻し (53 件)` がモノレポ main に着地。経路は add/update ではなく登録者数キャッシュ書き戻し (`channel_store.py:238`) だが、同一の `_git_commit` 機構 (パス明示 `git add -- <abs_path>` + `git commit -- <abs_path>`) であり、変更は `ytcheck/tasks/youtube-channels.json` のみ = 他プロジェクトの dirty 巻き込みなし。「ytcheck 自動 commit がモノレポに載る」実弾として成立
...
**最終更新**: 2026-07-15 (チケット #85: §8 ytcheck 自動 commit 実弾検証 OK を反映 — 45a7c39 登録者数キャッシュ書き戻し 53 件がモノレポ main に単独着地、`_git_commit` パス明示で巻き込みなし。§8 残りは .archive/split-era-git 削除の OWNER 承認のみ。前回 2026-07-14: §8 vault auto-sync 検証 OK を反映 — auto-sync commit 7/12〜7/14 毎日着地 + HEAD = origin/develop で push 貫通。残りは ytcheck 自動 commit のイベント待ちと .archive 削除承認のみ。前回 2026-07-10: 初版 + code-reviewer 指摘反映: §4 spot check を第 2 親経由に修正・`git log -- <dir>` 誤認防止注記、§5 突合を pre/post diff に機械化、§9 rollback を mv 退避に変更、§7 grep の vault/logs 除外、Phase 2 の bot 停止注記。レビューは git 2.34.1 実機 + テスト repo で Phase 1 レシピ再現実行済み)
