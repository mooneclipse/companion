...
- **セリフ枠（キャラ隣 `#quote-text`）**: 30 秒ごとに 1 言ずつフェード切替で表示（既存 `QUOTE_INTERVAL_MS=30s` / `.is-fading` transition は流用）。ローテーション内容は **helper の `GET /quotes` を集約点**にして bot 通知と dashboard ブラウザで完全一致させる:
  ...
  - **helper `/quotes` 集約 (2026-05-29 改修)**: helper (`server/nowplaying-helper.py`) が JST 当日の日付を key に in-memory cache で 1 日 1 回だけ build → 同じ JSON を返す。返り値: `{"date": "YYYY-MM-DD", "weather": [...lines...], "fortune": "...", "news": [...lines...], "anthropic": [...lines...]}`。app.js（ブラウザ）も `bin/dashboard-notify-ren-quotes.py`（bot 通知）も両方 `/quotes` を fetch する **薄いクライアント** に統一。占い random seed / 天気 fetch 時点 / news fetch 時点のズレは helper 側で吸収され、同日中は両者で「同じ占い文 / 同じニュース並び / 同じ天気行」が出る。
  ...
  - **Anthropic 新着リリース (2026-05-29 追加)** (helper 側): `https://www.anthropic.com/news` を helper が **ブラウザ UA** で取得（公式 RSS / Atom / sitemap が無いため HTML パース。default urllib UA は Cloudflare に 403 で弾かれるため UA 明示。memory: reference_mintupgrade_cloudflare と同系）。改版に強い `href="/news/<slug>"`（ルーティング由来）と各ブロックの **カテゴリ語** を抽出し、**カテゴリ `Product` のみ通す**＝新モデル/製品発表に限定（資金調達/採用=`Announcements`・オフィス開設・ポリシー等は除外）。タイトルは記事ページの `<meta property="og:title">`（SEO 必須で最安定）から取得。`/quotes` は `anthropic: ["Anthropic：<title>"]` 形式で返す（通常 0 件、新着のあった朝だけ 1〜3 件）。**ユーザー要望**: 「anthropic のリリースがあったらタイトルだけを bot と dashboard の ren セリフでお知らせ」。bot 側は無改修（既存 socket 転送が `anthropic` 行をそのまま流す）。**新着判定は slug 差分**: `.state/anthropic-news-state.json` に既出 slug を保存（`{"date","shown","seen"}`）、前回 build 以降に出た slug を新着とする（日付パース/TZ 非依存）。**state を 1 回引いて確定**（CLAUDE.md 準拠、stderr 文言マッチや retry ループを作らない）。**初回 build は無音 seed**（現状の全 slug を seen に取り込むだけで過去記事を一斉通知しない）。index fetch 失敗時は state を進めず空（既存 no-retry 方針）。同日再 build / helper 再起動は state.date で冪等。**`Product` のみ絞りの既知トレードオフ**: モデル発表が `Product` 以外（例 `Announcements`）で出た場合は拾えない → その時は `ANTHROPIC_CATEGORIES` を 1 行広げる。**Anthropic 発表のタイミング**: 公式の固定スケジュールは無いが米国太平洋時間の日中（平日中心）＝日本時間の深夜〜早朝に出揃うため、朝 5:30 の一括通知に「前回以降の新着」を載せる形で実用上ほぼ取りこぼさない（日中リアルタイム push は新規 cron が要るため不採用＝ユーザー選択）。
  - **CORS 回避**: file:// origin から外部 RSS への直 fetch は CORS 制約で弾かれるため helper proxy 1 段で受ける。改修後は天気 / 占い / news / anthropic が全て helper 集約になったため、ブラウザの quote frame は **外部 API を一切叩かない**（weather パネル用の Open-Meteo fetch は別系統で残置）。
  - **失敗時の振る舞い**: helper unreachable / parse 失敗 / fetch 失敗は該当配列が空（または fortune が空文字）で返る。app.js は queue 空のまま cycle 静止、notify script は exit 0 で抜けて翌日再試行（`.state/last-notify-date` を mark しないため）。retry/backoff は一切持たない（次の起動・次の日付ロールオーバーで再 build）。**weather のみ例外 (2026-07-14、#104)**: 取得失敗時は空配列ではなく告知行 1 本（「きょうの天気：取得できなかった…」）を入れて返し、当日 cache に確定しない＝次の `/quotes` 呼び出しで再 build（Done 2026-07-14 参照）。
...
- 2026-07-14 朝報の天気欠落対策: `/quotes` で weather 取得失敗を当日 cache に確定しない + 失敗を stderr に記録 + セリフで告知（チケット #104）。**事象 (2026-07-14 朝に実測)**: 5:30 起動時の初回 `/quotes` build で `_fetch_weather()` の Open-Meteo fetch が一過性失敗 → `except (OSError, ValueError): return None` で無言で握られ、weather 空の payload が JST 当日 key の cache に確定 → 同日中は再取得されず Telegram 朝報と TV セリフ枠の両方から天気が消えた（`.state/morning-report.json` が `"weather": []`、fortune/news は正常 = 回線全断ではなく weather fetch のみの失敗）。**変更 3 点**（`server/nowplaying-helper.py` のみ、`_build_weather_lines` / `_wx_band_stats` 等のロジック本体・`/quotes` handler の except fallback は不変）:
  - ① **失敗時 cache しない**: `quotes_payload()` で `_build_weather_lines()` が空（土日も最低 1 行出る設計なので「空 = 取得失敗」で確定できる）のとき、payload は組んで返すが `_quotes_cache` に確定しない。次の `/quotes` で再 build し、成功したとき初めて cache。**設計判断: failure を当日 cache に確定しない = 回復は次リクエストの再 build に委ねる。client retry / backoff は導入しない**（既存 no-retry 方針と整合、対症療法 2 周目ガードの retry ladder を呼ばない）。再 build の安全性: fortune は日付 seed の deterministic、news は TTL cache (`news_items()`)、anthropic は `state.date == 今日` なら shown を返す idempotent 設計で、再 build しても本文はブレない。消費者の呼び出し頻度は 5:30 の notify 1 回 + ブラウザ起動時 1 回程度で、cache しなくても外部 fetch 再走の実害なし。
  - ② **失敗の表面化**: `_fetch_weather()` の except で stderr に 1 行 `weather: fetch failed: {e!r}`（anthropic-news の既存ログと同形式、journald 行き）。挙動分岐・retry は増やさないエラー表面化専用（CLAUDE.md 準拠）。次回発生時に失敗種別（timeout / DNS / HTTP エラー）を確定できる。
  - ③ **取得失敗をセリフで告知**: weather 空のとき `WEATHER_UNAVAILABLE_LINE`（「きょうの天気：取得できなかった…TV の天気パネルの方を見てみて」）を 1 本入れる。cache 判定は取得成否のフラグで行い、告知行で non-empty になっても cache しない。notify script / app.js は無改修（weather 配列の文字列を素直に並べる薄いクライアント）。TV の天気パネルは `/quotes` と別系統（app.js が Open-Meteo 直 fetch、last-good 保持）なので参照先として正しい。
  - **検証 (2026-07-14)**: `py_compile` OK。正常系 = helper 一時起動 → `/quotes` 2 回で weather 3 行（平日）+ 2 回目が同一 JSON（cache）。失敗系 = urlopen 差し替えのユニットテスト（news/anthropic は patch out でネットワーク・本番 state 非接触）で (a) 告知行 1 本 (b) cache 未確定 → 失敗→成功遷移で実天気に置き換わり cache 確定・fortune は前後同一 (c) stderr ログ、の全 check pass。restart はしない（dashboard.service は 09:00 停止済み、明日 5:30 起動から新コード有効）。
  - **波及 2 点（code-reviewer 指摘、いずれも許容と判断）**: (1) 失敗朝は notify の `save_morning_report` 経由で告知行が `.state/morning-report.json` に載り、日中の先回り発話 (`maintenance/scripts/proactive-companion.sh`) の「weather 非空 = 有効な朝報」ゲートを貫通して `morning_weather` = 告知行 + `morning_hint` 有効で prompt に流れる。朝報で実際に届けた文言そのものなので「今朝知らせた内容を踏まえる」という #36 の意味論は保たれており許容（文言マッチでの除外は CLAUDE.md 違反のため組まない）。(2) 失敗朝は「bot 通知と TV 表示の完全一致保証」が成り立たない（通知 = 告知行、ブラウザ = 再 build 成功なら実天気）。意図した劣化として helper / notify 両 docstring に例外を明記済み。
- 2026-07-10 朝 BGM 7 曲入れ替え（チケット #78「朝のダッシュボードbgmの曲を入れ替えて欲しい」、方向はユーザー指定「完全おまかせ」）。**対象はファイルのみ、コード・設定変更なし** (`dashboard-start.sh` は `~/music/<slot>/` を mpv がフォルダ再生するため配置換えで完結)。新セット (NCS 定番アップリフト系、7 アーティスト 7 曲): Elektronomia - Sky High / Syn Cole - Feel Good / DEAF KEV - Invincible / Lensko - Let's Go! / Jim Yosef - Link / Diviners - Savannah / Culture Code - Make Me Move。旧セット (2026-05-13 の 7 曲) は削除せず `~/music/.archive/morning-20260513/` へ退避 (CREDITS.txt 込み)。取得は ncs.io トラックページの `data-url` 直リンク (`~/.claude` memory `reference_ncs_download` の手順)、`file` で MP3 実体確認 + mpv `--ao=null` 先頭 2s デコード検証 7/7 OK。新 `~/music/morning/CREDITS.txt` に slug 一覧と **slug 衝突の罠** (ncs.io/hope・/eclipse は同名別アーティスト曲、追加時は `<title>` でアーティスト検証必須) を記録。次回 05:30 本番発火で新セット再生開始 (dashboard.service は日中非稼働のため restart 不要)。
- 2026-07-02 dashboard-start.sh 冒頭に `systemctl --user stop screensaver.service` を追加（screensaver プロジェクト新設に伴う双方向排他）。screensaver (09:05〜05:25、`~/companion/screensaver/`) 側の start.sh には dashboard 稼働中 skip ガードが既にあり、こちらは逆方向 = 05:25 の screensaver-stop.timer が失敗した朝に HDMI-1 kiosk firefox が 2 重になるのを防ぐ。冪等 (非稼働なら no-op)。`Conflicts=` は対称動作（screensaver の起動が dashboard を落とす）のため不採用。code-reviewer (screensaver 全体レビュー) の指摘採用。
- 2026-06-12 Anthropic 新着通知の取りこぼし修正（チケット #15「本体セッションで Anthropic モデル新製品通知機能の設定状況を確認・必要なら再設定」）。**事象**: Claude Fable 5 / Mythos 5 の発表（`claude-fable-5-mythos-5`）が朝の ren セリフに出なかった。**根本原因は独立した 2 層**:
  - **① カテゴリ運用の変化**: 発表記事のカテゴリが `Announcements` だった（2026-05-29 設計時の既知トレードオフ「launch が Product 以外で出た場合は拾えない」が発火）。実測で直近 index 10 件中 `Product` は 1 件のみ（opus-4-8）で、モデル発表も `Announcements` で出る運用に変わっている。**設計判断**: 文書化済みの対応「`ANTHROPIC_CATEGORIES` を広げる」の単純適用（Announcements 全通し）は提携/資金調達/オフィス開設ノイズ（直近 2 週で 8 件）でユーザー要件「製品・モデル発表のみ」を崩すため不採用。`_anthropic_passes(slug, category)` を導入し **Product は無条件 + Announcements は slug に "claude" を含むもののみ** 通す。実 index でのドライラン: fable-5 / claude-corps / opus-4-8 = PASS、glasswing / dxc-alliance / series-h / milan 等 = drop。既知トレードオフ（更新）: Claude 非冠の新製品が Announcements で出た場合は拾えない → その時は `_anthropic_passes` を 1 箇所直す。
  - **② fetch timeout の実測不足**: `ANTHROPIC_FETCH_TIMEOUT = 3.0` が実測（index ~390KB、この回線で wall 3.4〜13.0s、read timeout 頻発）に届かず、2026-06-11/12 の 2 朝連続で index fetch が read timeout → state が 06-10 で停滞（journal 上は無音。例外を握り潰す設計だったため事後診断不能だった）。3.0→10.0（per-op）に実測ベースで確定。**2 周目ルール判断**: 連動する `QUOTES_TIMEOUT`（notify 側）は 10→15 に次ぐ 2 度目の数値変更にあたるため設計レベルで再検討した。根本構造（helper がハンドラ内で直列 fetch、呼び出し側予算 = worst 合算）の引き直し（build 事前化等）は個人ダッシュボード規模に過剰で、失敗原因は構造でなく「実測なしで置いた初期値」にあるため **「2 周目を許容して合理化」を選択**。あわせて予算式を「timeout 合算」から「観測 wall worst 合算」（urllib timeout は per-op の stall 検出で総時間上限ではない、と判明したため）に組み替え、15→90 で確定。notify script は detach 起動の fire-and-forget で遅延依存の後続なし。
  - **③ 無音破綻の表面化**: `_anthropic_index` の fetch 失敗 / パース 0 件時に stderr 1 行（journald 行き）を print 追加。挙動分岐・retry は増やしていない（エラー表面化専用、CLAUDE.md 準拠）。
  - **検証 (2026-06-12)**: `py_compile` 両 .py OK。実 index ドライラン（state 不変）でフィルタ判定とログ出力を確認。dashboard.service は 9:00 停止済みのため restart 不要、明朝 5:30 起動で新コード反映。
  - **残（観察対象）**: 明朝 6/13 の build で未 seen の `claude-corps`（PASS）が通知され、`dxc-anthropic-alliance`（drop）が無音 seed されるはず → 実機で新フィルタの初観測。`claude-fable-5-mythos-5` 自体は既に seen 済みのため遡及通知はされない（ユーザー把握済み）。
- 2026-05-29 Anthropic 新着リリースのタイトルを ren セリフ（bot 通知 + dashboard ブラウザ）に追加。**ユーザー要望**: 「anthropic のリリースがあったらタイトルだけを bot と dashboard の ren で知らせてほしい。Anthropic の発表はいつ何時に出るのか？」。設計確定: AskUserQuestion で対象=「製品・モデル発表のみ (anthropic.com/news)」/ タイミング=「朝 5:30 の一括に追記」を選択。
  - **Anthropic 発表タイミングの回答**: 公式の固定スケジュール無し。配信元は anthropic.com/news（RSS 無し）と platform.claude.com の release-notes（日付付き changelog）。曜日はほぼ平日、時刻は米国太平洋時間の日中 = JST 深夜〜早朝（PT 10:00≒JST翌02:00 / PT 13:00≒JST翌05:00）。よって朝 5:30 一括で前夜の米国発表をほぼ取りこぼさない。
  - **実装**: helper (`server/nowplaying-helper.py`) に `_anthropic_index` / `_anthropic_title` / `_anthropic_new_titles` を追加。anthropic.com/news をブラウザ UA で取得（default UA = 403 を実測、Cloudflare）、`href="/news/<slug>"` + カテゴリ抽出 → カテゴリ `Product` のみ → slug 差分で新着判定 → 記事 `og:title` 取得。`.state/anthropic-news-state.json` で state を 1 回引いて確定（初回無音 seed、fetch 失敗は state 不変、同日/再起動は date で冪等）。`/quotes` に `anthropic: ["Anthropic：<title>"]` を追加。
  - **consumer**: `bin/dashboard-notify-ren-quotes.py` は `weather+fortune+news` の後に `anthropic` を append（`QUOTES_TIMEOUT` 10→15 に初回サイズ調整。Anthropic index+記事 fetch 追加分）。`web/app.js` `buildQuoteQueue` も news の後に `anthropic` を push。bot 側コードは無改修（既存 socket 転送がそのまま流す）。
  - **設計判断 (Product のみ絞り)**: anthropic.com/news は資金調達・オフィス開設・ポリシー等の企業ニュースも混在するため、ユーザーの「製品・モデル発表のみ」選択に合わせカテゴリ `Product` だけ通す。launch が `Product` 以外で出た場合は拾えない既知トレードオフ → `ANTHROPIC_CATEGORIES` を広げて対応可（場当たり patch ではなく文書化した単一判断）。
  - **検証 (2026-05-29)**: `py_compile` 両 .py OK / `node --check web/app.js` OK。helper 関数の単体テスト（一時 state）で ① index 11 件パース・カテゴリ判定正確（opus-4-8/claude-design=Product、series-h/milan=Announcements で除外）② 初回 seed 無音（[] 返し・seen 11 件記録）③ 同日 cache ④ Product slug を seen から外し date を前日に戻す → og:title `Introducing Claude Opus 4.8` を新着検出・seen 再追加 ⑤ `quotes_payload` に `anthropic` list フィールド、を確認。本番 `.state/anthropic-news-state.json` は未生成（テストは一時 state、初回実 build で無音 seed される）。
  - **残**: 〔ユーザー〕本番初回 build は無音 seed のため Anthropic 行は出ない（過去記事一斉通知防止）。**実機での新着表示確認は次に新しい Product 記事が出た朝まで観察対象**。それまでに動作を強制確認したい場合は `.state/anthropic-news-state.json` の `seen` から直近 Product slug（例 `claude-opus-4-8`）を 1 つ削り `date` を前日に書き換えて `systemctl --user restart dashboard.service`（helper 再起動で次 `/quotes` build 時に新着扱い）。helper 編集は restart まで無反映 (memory: reference_remote_server_restart と同系)。
