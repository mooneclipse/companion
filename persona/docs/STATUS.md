...
**線引き更新 (2026-07-13、チケット #96、OWNER 承認)**: 上記「出どころは実活動限定」に **1 例外だけ**認める — **実際にやった調査の中で派生した関心** (「○○を調べてたら△△が引っかかった」) は実活動由来と見なし、investigate 分岐の claude が index に自己登録できる (`[[interest: 話題 | 出どころ]]` marker、bot が剥がして `source="derived"` + `origin=調べたノート名/URL` で登録)。出どころフィールドは marker 形式で**必須** = 欠けたら登録しない (実体のない捏造との区別を形式で強制、fail-closed)。増殖は 1 発火 1 件 + 既存 MAX_THREADS 押し出し + 既存 decay で抑制 (新しい上限機構は足さない)。調査をしない ticket / remind / talk には指示を出さない (例外を 1 分岐に閉じる)。これで調査→派生関心→次の調査の自己ループが回り、index が初めて claude 自身の関心を含むようになる (#93 思考ログ自由記述化とセット、2026-07-13 bot 全体レビュー・自律の心臓部)。
...
persona/ は 2026-07-11 から **`~/companion/` モノレポ配下** (チケット #82 — 本台帳 1 ファイルが git ゼロだったため rollback 価値を根拠に track 対象へ)。個別 git init の判断は不要になった。外したくなったら root `.gitignore` に `/persona/` を足すだけ (移行手順書 `workspace/redesign/monorepo-migration.md` §2)。運用の正は `~/companion/CLAUDE.md` git 運用方針。
