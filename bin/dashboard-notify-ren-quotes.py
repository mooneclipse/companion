...
def _clothes_phrase(hi):
    # web/app.js の _clothesPhrase と同じ閾値・同じ文言
    if hi is None:
        return ""
    if hi >= 30:
        return "半袖でも暑そう"
...
def _build_weather_line(label, stats):
    if not stats:
        return None
    parts = []
    if stats["hi"] is not None:
        ...
        parts.append(t)
    clothes = _clothes_phrase(stats["hi"])
    if clothes:
...
def build_weather_lines(now):
    """平日 [朝 7-9, 夜 18-22] / 土日 [一日 6-21]。data 不在は空 list。"""
    data = _fetch_weather()
    if not data:
        return []
    ...
    if is_weekend:
        ...
        line = _build_weather_line("きょうの天気", all_band)
        if line:
            out.append(line)
    else:
        morning = _wx_band_stats(data, today_ymd, 7, 9)
        ml = _build_weather_line("朝の天気", morning)
        if ml:
            out.append(ml)
        evening = _wx_band_stats(data, today_ymd, 18, 22)
        el = _build_weather_line("夜の天気", evening)
        if el:
            out.append(el)
    return out
