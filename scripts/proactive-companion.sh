...
# 越えない)。FLOOR は静かな日の最低発火率。FRESHNESS_DAYS = 活性度を測る新鮮さ窓 = この
# ケイデンスの唯一の窓 (step7 で decay と activity_score の両方にこの 1 値を渡す。bot.py の
# 滲ませ用 decay TTL=14=PROACTIVE_INTEREST_TTL_DAYS とは別管理)。FLOOR を base と等しくすれば
# 変調幅 0 = 従来の固定ケイデンスに戻る (env で戻せる経路)。
