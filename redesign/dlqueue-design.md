...
- `POST /api/dl` の入口で **urlguard.normalize_dl を必須通過** (2026-06-12 変更: 再生の門 normalize から、streaming 専用 host = TVer を除外した DL 専用の門へ。video-design §4.1)。弾けば 400。queue には normalize 済み URL しか入らないため、worker は検証を再実装しない (門を 2 重化して drift させない)。TVer を DL に流さない理由 = 期限付き見逃し配信のローカル保存は期限・権利の両面を迂回する (RV-11 判断)。
...
- v1.3 (2026-06-12): §2.3 の門を `urlguard.normalize_dl` へ変更 (TVer 対応 = RV-11 に伴う分離。再生は TVer 可、DL は不可)。詳細は video-design §4.1 / 改版履歴 2026-06-12 エントリ。
