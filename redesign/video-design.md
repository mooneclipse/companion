...
- **不採用候補**: (α) `--hwdec=auto` は driver 自動選択が将来 driver 入替時に挙動を変える → `vaapi` で固定。(β) `--hwdec=vaapi-copy` はメモリコピー経由で省 GPU メモリだが Ivy Bridge GEN7 では VRAM 共有ゆえ利点なし、本筋の zero-copy `vaapi` を採用。
...
- **検証**: service edit + `systemctl --user daemon-reload` で ExecStart に `hwdec=vaapi` 反映を確認。**実 restart は本対応では実施しない**(現プロセス稼働中、user が Live URL 再投入時に frame-drop / hwdec-current の再判定段を lead が握る)。
