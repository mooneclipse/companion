...
### 非目的
~~キュー/プレイリスト(MVP punt、capability 拡張は §5 第2原則で新 endpoint)~~ → **2026-06-18 RV-12 で解除**(§5 第2原則どおり server stateless を崩さず新 endpoint `/api/video/play_playlist` + `/api/video/queue/{next,prev,jump}` で実現。mpv ネイティブ playlist + flat-playlist メタ展開、title は PWA 保持。契約 C1〜C5 は改版履歴 2026-06-18 参照)/ live の DVR シーク開放(初手無効、build 後に1回判断)/ ライブ先頭へ戻る / 字幕・画質手動切替 / YouTube・ニコニコ動画以外のサイト(ニコニコは 2026-06-11 追加、§4.1 と改版履歴参照。ニコ生 `live.nicovideo.jp` は引き続き非目的)/ Phase 4 キャラクター層。
...
- **残課題(本オプション範囲外)**: hwdec=vaapi 投入後も高 bitrate Live (3D 配信等) で 5 秒周期の固まりが残っていたが、Wi-Fi 2.4GHz の帯域上限が根本原因と切り分け確定。mpv buffer 調整は的外れ、真の対策はユーザー環境変更(5GHz/有線化/480p 受容)。RV-8 は 2026-06-29 クローズ。
...
- 不採用: deno(§4.3) / 非同期ジョブ基盤(§1、mpv が state 所有) / lane ガード・409(§3.2、replace 単一保持実証) / auto-stop タイマー(§7) / wmctrl 配置(§3.2、fs-screen) / ~~キュー(§0)~~(**2026-06-18 RV-12 で採用** = mpv ネイティブ playlist で監視スレッド不要、§1 stateless を崩さず実現。改版履歴参照) / live DVR シーク開放(§0、初手無効)。
...
- RV-12 プレイリスト再生キュー = §0 punt 解除 (2026-06-18): プレイリスト URL の ①中身可視化 ②次/前 ③自動連続再生(PWA を閉じても継続)を、§1 server stateless / 再生 state は mpv 所有 / 監視スレッド不要 を崩さず実現。採用方針 = **mpv ネイティブ playlist**(`loadfile append` で内部 playlist に積み、`keep-open=no` の eof→次エントリ自動 advance がネイティブに連続再生 = 監視スレッド不要)+ **`--flat-playlist` メタ展開**(`get_property playlist` は title を持たないため、flat 展開で得た title を PWA 側で `playlist-pos` と突き合わせ表示)。実装 = `server/playlist.py`(新規 flat 展開、dlqueue.py 同型の env 隔離/killpg/timeout/-4)+ `server/video.py` の `play_playlist_load`/`queue_{next,prev,jump}`(固定テンプレートのみ、§3.4 verb whitelist を維持)+ `/api/video/play_playlist`・`queue/{next,prev,jump}`(全 Bearer)+ PWA の list= 自動判定 + キュー UI。**設計契約(破ると黙って壊れる)**: C1(index 1:1 固定 = 投入後 server から playlist 非編集、操作は next/prev/jump のみ、再投入は loadfile replace で全消し→再構築。playlist-pos と PWA 保持 title 配列が永久に 1:1)/ C2(title は PWA 保持 = play_playlist レスポンスで渡し PWA がメモリ/localStorage 保持、server は新規 state なし。`force-media-title` で mpv に持たせる案は verb whitelist に外部サイト由来文字列を流す面を開くため却下 = §3.4 RCE 一線)/ C3(N=100 件で 1 回確定 = `yt-dlp --flat-playlist -I 1:100`、YouTube Mix `list=RD...` の疑似無限対策、数値は刻まない)/ C4(entry reject は skip = 各 entry url を `urlguard.normalize` に通し None なら 1 件だけ skip、展開全体を fail させない。「全 K 件中 M 件」を PWA 表示)/ C5(no-playlist= 依存の明記 = server 展開方式ゆえ mpv へ届く各 url は単一動画。万一 `watch?v=X&list=PL...` が渡っても mpv unit の `no-playlist=` 強制が無視する = 暗黙のセーフティネット。unit 本体は無改変)。`urlguard.py` は無改変(host 照合は既存 allowlist で足り、playlist 判定は別レイヤ)。bot 側は playlist 非対応のまま(urlguard 無改変ゆえミラー対象外)。検証 = 単体 110 件 + headless mpv(別 socket、本番非接触)実挙動 + Playwright UI スモーク 14、**TV 実機検証は user 在席時に残**。詳細は `remote/docs/STATUS.md` 2026-06-18 エントリ。
