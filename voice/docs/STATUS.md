...
- **キャラ**: ~~四国めたん (speaker_id=2、T-1 sprint #1 V-15 で `/speakers` API 実機確認済)~~ → **玄野武宏 (speaker_id=11) に確定・切替済み** (2026-06-12 persona 軸 2、本台帳冒頭 entry 参照。四国めたん id=2 は技術検証時の仮置きで現用なし)
...
4. **bot/ 側実装着手** ✅ **完了 (2026-06-12、Phase 4 voice bot 統合として実装。Done 同日 entry 参照)**:
   ...
   - ~~残 (user 操作): `voice/.env` の `VOICE_DEFAULT_SPEAKER` 2→11 切替、bot restart、V-S1/V-S2 実弾~~ ✅ **完了** (bot/docs/STATUS.md 2026-06-12 最終更新履歴「restart + speaker 11 + V-S1/V-S2 実弾 pass 済み」。`.env=11` は 2026-07-21 #125 整理時に実物再確認)
