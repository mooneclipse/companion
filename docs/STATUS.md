...
1. **voice/ 側実装着手** ✅ **完了 (2026-05-19)、v2.0.2 §5.3 voice/ 側前倒し**:
   ...
   - 残: engine の enable (常駐化) 判断 — bot 側 (voice_command.py) 着手段階 (Phase 2.5 T-D 後半完了後 = 2026-06 中下旬目処) で判断
...
（なし、2026-05-19 TODO #1 voice/ 側実装着手 + 完了基準 (i) 達成で voice/ 側前倒し分は完結。次は Phase 2.5 T-D 後半 (CreditBudgetGuard、bot/docs/STATUS.md、2026-06 上旬予定) → bot/ 側 voice_command.py 実装 (2026-06 中下旬目処))
