...
- **commit**: `05064d9`。restart で本番反映、push は user 承認待ち
...
- **commit**: `c9caaa8`。restart 済み (20:10 JST)、push は user 承認待ち
...
- **未消化**: restart (`systemctl --user restart companion-bot`) で本番反映。**実機の前景提案発火 (#chat に「○○やっとこうか?」が滲む) を成果物で実見待ち** (関心 index に外向き衝動を誘う signal が溜まってから自然待ち)。push は user 承認待ち。bot.py 改変は非自明 = 様子見観察対象。
