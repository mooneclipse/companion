...
最終更新: 2026-05-28 (RV-5 build 検証 In progress: V-A1/V-A3 VOD pass / V-A6 RAM 実測 / Live 60fps frame-drop 観測 → hwdec=vaapi + video-zoom=-0.05 を service に追加、restart 待ち)
...
- [~] **build 検証(GUI/実機、SETUP §6) — In progress (2026-05-28)**: V-A1 ✅(2026-05-20) / V-A3 ✅ VOD 部分(2026-05-20) / **V-A3 Live 60fps**: hwdec=no で frame-drop 690 / estimated-vf-fps 27.3 (container 60.0) を観測 → `--hwdec=vaapi` を service に追加・daemon-reload 済、user の Live URL 再投入で再判定予定 / **V-A6 ✅ RAM 実測**: 183-244 MB(3.7Gi の 6.5%、24/7 常駐妥当性 OK)。**video-zoom=-0.05 オーバースキャン補正導入**: TV 実機で「ちょうどいい」確認済。🟠 dashboard BGM mpv 同居 / 720p deinterlace / 単一 lane 再確認は残。**TV が光る/音が出るので朝の dashboard 稼働中(5:30〜9:00)は避け、ユーザー在席時に lead が握る**
...
- **RV-5 build 検証**: V-A1 ✅ / V-A3 VOD ✅ / V-A6 RAM 実測 ✅(183-244 MB)/ V-A3 Live 60fps は hwdec=vaapi 投入後の user 再投入で frame-drop 再判定待ち。video-zoom=-0.05 はオーバースキャン補正で導入(user OK)。
