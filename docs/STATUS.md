...
- [~] **RV-12: プレイリスト再生キュー → 2026-06-18 server+PWA 実装/commit 済、TV 実機検証残** (元の 2 件ロードマップ外、`video-design.md` §0 が punt 予告していた拡張を user 要望で着手)。mpv ネイティブ内部 playlist + flat-playlist メタ展開 (title は PWA 保持) で server stateless を崩さず実現。出口は list= 自動判定。**headless mpv + Playwright UI スモークまで完了、TV 実機 (自動 advance / PWA 閉じて継続 / 再開 pos 復元 / live 混在停止) + restart は user 在席時に残**。下記 Done 参照。
...
- 2026-06-18 **RV-12 プレイリスト再生キュー (server + PWA) 実装・commit (user 要望: プレイリスト URL で ①中身が見える ②次/前 ③1 曲終わったら自動で次へ、PWA を閉じても継続)**: `video-design.md` §0 が「キュー/プレイリストは MVP punt」と予告していた拡張。**設計の背骨 (§1 server stateless / 再生 state は mpv 所有 / 監視スレッド不要、§3.4 verb whitelist) を崩さず実現**するのが肝。採用方針 = mpv ネイティブ playlist (`loadfile append` + `keep-open=no` の自動 advance) + `--flat-playlist` メタ展開で title は PWA 保持。
  ...
  - **残 (user 在席時)**: 実プレイリストの flat 展開 (url 形式 / title 欠落 / 展開時間) を既知の小プレイリストで実測 + **TV 実機検証** (投入→1 曲目→自動 advance→次/前/jump→PWA 閉じても継続→再開で pos 復元→live 混在で停止) + `systemctl --user restart companion-remote.service` (server/*.py 反映)。urlguard は無改変 (host 照合は既存 allowlist で足り、playlist 判定は別レイヤ)。
  - **台帳同期 (残)**: `workspace/redesign/video-design.md` §0 非目的の「キュー/プレイリスト punt」解除 + C1〜C5 契約の改版履歴反映 (workspace repo、別 commit)。bot 側は playlist 非対応のまま (urlguard 無改変ゆえミラー同期不要)。
