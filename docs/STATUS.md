...
最終更新: 2026-05-28 (RV-8 切り分け (c)(d) claude 側調査完了 = 現状 mpv は cache/readahead/stream-buffer 一切未指定で 0.34.1 デフォルト稼働、yt-dlp は URL 解決のみで chunk fetch は mpv 内 ffmpeg。仮説と (a)(b) 実測コマンドを RV-8 ブロックに追記、修正コミットは (a)(b) 実測後に判断)
...
- [ ] **RV-8: Live edge 追従 stall (5 秒周期カクつき) の root cause 切り分け**: hwdec=vaapi で平均 fps は 60.0 まで回復したが、user 実機で 5 秒周期(YouTube Live DASH/HLS segment 長と一致)の固まりが残る。機械測定で `demuxer-cache-state.underrun=True` 発火 + `paused-for-cache` 反復 + estimated-vf-fps が 60.0↔37.5 を反復 = **ネットワーク受信(yt-dlp chunk 取得)がデコードに追いついていない**ことを確認。**対症療法 2 周目ガード抵触リスク高(`--ytdl-format` 480p 絞り / `--cache-secs` 数値変更 はいずれも「数値だけを動かす」修正)** なので、独断で打たず root cause 切り分けを先行する。
  ...
  - **次に user 機 (TV 直結マシン) で取るべき (a)(b) 実測コマンド** (claude 環境では再現不可):
    - Wi-Fi リンク品質: `iwconfig 2>/dev/null | grep -E 'ESSID|Quality|Signal|Bit Rate'` (live 再生中 1 回 + 再生 60 秒後にもう 1 回、変動も見る)
    - YouTube CDN への jitter / loss: `ping -c 30 youtube.com` と、journalctl から実際の CDN edge を抜く `journalctl --user -u companion-video-mpv -n 500 2>/dev/null | grep -oE 'r[0-9]+---sn-[a-z0-9\-]+\.googlevideo\.com' | sort -u` → 出た host へ `ping -c 30 <host>`
    - 再生中の throughput 帯: `sudo iftop -i wlp* -P -B` または `nethogs wlp*` で mpv プロセスの実トラフィック (sustained MB/s vs HLS peak)
    - mpv 側 cache 観察: `socat - UNIX-CONNECT:/run/user/1000/companion-video-mpv.sock` で `{"command":["get_property","demuxer-cache-state"]}` を 1-2 秒間隔で叩き、`underrun` / `eof` / `fw-bytes` の遷移を 60 秒録る
