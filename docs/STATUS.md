...
## 第 6 作「さぐり」（着手、2026-06-08 / `/newgame` 4 度目、Steam 実データ起点 + ユーザー引数でジャンル固定）
...
最終更新: 2026-06-04 (**第 5 作「ともる」着手＝`/newgame` 3 度目**。ユーザー要望「あかりとは別ジャンル」→ Steam 行動データのフレッシュ再分析でデッキ構築を外し能動クラスタ4レーンを発散、game-critic で B/D（リプレイ性ゼロ）を却下・A（実機fps/親指オクルージョン/理不尽死で実装不確実）を次点保留し **C「ともる」＝リアルタイム採掘/精錬サバイバル（縦坑ダイブ・灯量＝視界＝酸素＝帰路の三位一体）を選定**。あかり当たり核「明度＝リソース」を別ジャンルへ正統発展。下記「第 5 作『ともる』」section 参照。直前の更新＝**第 4 作「あかり」感想 = 7/10** 受領＝シリーズ初の当たり**（5→3→3→7）。Steam 実データへの方向転換が正解と実証。良かった＝スレスパの落とし込み/"気を抜くと負ける"難易度/「灯」独自システム。課題＝初接触 UX 往復・敵 1 パターンで何度も遊べない・分かりにくいカード。ユーザーが ver2 制作の可能性を明言→**「あかり ver2 設計起点メモ」section に資料集約**（ver2 最優先＝リプレイ性＝敵バリエーション）。出荷後 v1.1〜1.3 で UX/実機バグ修正済み、その穴を受け `/newgame` に面白さチェッカー（企画 devil 常時化＋playtester 実機リアリズム強化）追加。下記「第 4 作 方向転換」section 参照)

## 第 5 作「ともる」（着手、2026-06-04 / `/newgame` 3 度目、Steam 実データ起点）
