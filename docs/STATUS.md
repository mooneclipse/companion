...
- **commit**: `0e85b67`。**restart + セットアップは user 操作待ち**: (1) supergroup に memo topic 作成、(2) topic の thread_id を `.env` の `BOT_THREAD_ID_MEMO` に設定 (未設定なら機能ごと無効 = 先行 restart しても安全)、(3) bot を supergroup 管理者にして can_delete_messages 付与、(4) `systemctl --user restart companion-bot.service`。反映後の実弾確認: メモ投稿 → 👌 reaction + notes/ ファイル生成 (0o600) → 編集 → ファイル追従 → 48h+1h 後に topic から消える
