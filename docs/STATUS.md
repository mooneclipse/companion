...
### ニコニコ動画 残課題 (2026-06-11、allowlist 済み・実再生は未解決)
- [ ] **RV-9: ニコニコ動画の実再生 (mpv 0.34.1 ytdl_hook の cookies 未対応)**: allowlist 拡張 + テストは完了 (下記 Done 2026-06-11) したが、mpv 実再生スモークは resolving 40〜45s → idle で **playing 不達 (2 回再現)**。真因 = ニコニコ domand 配信 (delivery.domand.nicovideo.jp) は yt-dlp の **per-format `cookies` フィールド (domand_bid)** を要求するが Cookie は `http_headers` に乗らず、**mpv 0.34.1 ytdl_hook は `cookies` フィールド未対応** → HLS playlist 取得 403 → load 失敗。実証 = hook 同一引数の yt-dlp 直叩き rc=0 / m3u8 を cookie なし GET=403・cookies 値を Cookie ヘッダ付与 GET=200。**対症療法 (リトライ/分岐追加) 禁止 (orc 指示)** につき、対処方針 (mpv 更新 / hook 差し替え / 見送り 等) は設計判断待ち。**RV-10 (事前ダウンロードキュー) 完了後に要否を再判定** — ローカル保存ファイルの再生なら ytdl_hook を通らず cookie 問題ごと迂回できるため (下記ロードマップ参照)。
  - **2026-06-12 RV-10 完了につき再判定材料**: sm9 を yt-dlp 直 DL (1135s) → play_local で playing 到達を実証、**事前 DL 経路ならニコニコは観られる**。残る RV-9 のスコープは「ニコニコの即時ストリーミング再生」のみ。ニコニコ DL は実測 19 分/5 分動画 (ニコニコ側の帯域制限とみられる) で「即観たい」用途には遠いが、対処候補 (mpv 更新 / hook 差し替え) はいずれも churn リスクが大きい。**要否は user 判断待ち** (不要なら RV-9 をクローズして台帳に見送り根拠を記録)。
