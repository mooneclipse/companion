...
最終更新: 2026-05-20 (F-video 実装完了 RV-1〜RV-5 / build 検証は GUI・実 live URL 依存で残)
...
### F-video 追加要望（次セッション以降、user 2026-05-21。bot 非依存 = v1-α 系列）
- [ ] **RV-6 URL 入力欄のクリアボタン**: `#video-idle` にクリアボタンを足し、`#video-url` を空に + 取りこぼし/失敗文言(`#video-ended`)もクリア。純 client UI、低リスク。
- [ ] **RV-7 ±N秒スキップ + ステップ可変**: 再生中に「10秒進む / 10秒戻す」ボタン。上下ボタンでステップ秒数を可変（既定 10s）。
