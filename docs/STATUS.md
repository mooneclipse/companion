...
- [ ] **RV-10: 事前ダウンロードキュー (外出先から投入 → 帰宅後ローカル即再生)**: Telegram bot か remote PWA から URL をキュー投入 → 自宅機で yt-dlp が 720p 上限でローカル保存 → PWA の「DL 済みリスト」から `loadfile <ローカル path>` で再生。yt-dlp 解決 40〜70s (video-design §2、ネットワーク律速で短縮不可) がローカル再生では消えるため、**待ち時間短縮の本命**。着手時に軽い設計ラウンドを 1 回挟む。主要論点: 新 endpoint 群 / 保存先と HDD 容量上限・古いファイルの掃除 / urlguard と同一 allowlist 適用 / DL 完了通知 (Telegram) / yt-dlp 直叩きになるため env 隔離・known-good churn 台帳 (§8) との整合 / ローカル path の loadfile を許す際の境界 (任意 path 再生にしない)。ニコニコは yt-dlp 直 DL なら cookies を yt-dlp 自身が処理するため **RV-9 の 403 を迂回できる見込み** → 完了後に RV-9 要否を再判定。
