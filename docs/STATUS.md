...
- 2026-05-22 小箱キャラを下半分中央に追加（commit `0591f85`）。設計ノート `~/companion/vault/notes/2026-05-16_dashboard-character-design.md` を実装。
  ...
  - **配色フォロー（2026-05-22、ユーザーフィードバック）**: 「黒地にオレンジの目は視認性が悪い」→ `.cmp-screen` を `#0c1118`→`#1a2230` に上げた（窪みは body `#28323f` より暗く維持、目/口の `var(--accent)` #f3b85f は据置）。地を上げる/橙維持はユーザー選択。code-reviewer pass（WCAG コントラスト比 9.0、AAA 超）。実機 1080i 目視は同日テレビフル起動で実施予定。
