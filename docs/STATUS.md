...
- [ ] **RV-11: TVer 対応の実 URL 検証**: yt-dlp 2026.03.17 に TVer extractor は存在する (orc 確認 2026-06-11) が、DRM 化の進行・geo/会員制約は実エピソード URL でしか判定できない。user から見たい番組の URL を 1 本もらい `-J` で解決可否を確認 → 可なら allowlist 拡張 (ニコニコと同じ canonical ベクタ手順 + 実再生スモーク)、不可 (DRM) なら見送り根拠を台帳に記録。**検証が先、allowlist 追加はその後** (動かないものの門だけ開けない)。TVer は期限付き見逃し配信のため RV-10 (事前 DL) との組み合わせは権利・期限面を含め検証時に判断。
