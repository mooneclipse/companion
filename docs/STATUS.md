...
最終更新: 2026-05-22 (RV-6/RV-7 + 実機確認で出た不具合3件[レイアウト/pause継承/skipキャッシュ]修正)
...
- [ ] **build 検証(GUI/実機、SETUP §6)**: V-A1 `--fs-screen` TV 全画面 + idle 窓非占有 / V-A3 実 live URL の is_live・duration・seekable / V-A6 resident mpv RAM 実測。🟠 dashboard BGM mpv 同居 / 720p deinterlace / 単一 lane 再確認。**TV が光る/音が出るので朝の dashboard 稼働中(5:30〜9:00)は避け、ユーザー在席時に lead が握る**
...
- 2026-05-22 **F-video 追加要望 RV-6（URL クリアボタン）+ RV-7（±N スキップ/ステップ可変）実装完了**
  ...
  - 実機確認(2026-05-22, ユーザー在席)で 3 件の不具合を発覚 → 即修正:
    ...
    - ③ **±N スキップ無反応**: サーバ→mpv 経路は実機テストで正常(相対 seek で pos +30)、app.js wiring も正常。原因は端末 PWA の旧 app.js キャッシュ(skip ボタンのラベルは index.html 静的記述ゆえ表示≠新版実行)。**sw.js CACHE v5→v6 bump** で旧キャッシュ置換、リロードで反映。
  - 残: ①②③修正後の再リロード実機確認(±N スキップ実動 / レイアウト揃い / 再生が pause で始まらない)。朝 dashboard 稼働(5:30〜9:00)を避ける。
