...
### 自律ループ「動く = notes 自己調査」分岐 (2026-06-19、persona 軸 4 拡張 (3) 勝手な実行 A、commit までで停止 = restart/push は user 承認待ち)
...
- **未消化**: restart (`systemctl --user restart companion-bot`) で本番反映。**実機の investigate 発火による notes 新規作成 + #chat 報告の実見待ち** (発火窓 9-22 の確率発火 + interval due の合致を自然待ち、exit code でなく実 notes ファイルと報告で確定)。push は user 承認待ち。bot.py 改変は非自明 = 様子見観察対象。
