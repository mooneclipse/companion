...
- [ ] ユーザー実機プレイ（v1.1.0）→ 感想受領 → §9 記録（次作方向づけ）。
- [ ] **検討事項（ユーザー提起、2026-06-03、あかり一段落後に議論）**: 現行 `/newgame` には「面白いか」を客観チェックする専任が企画・製造後どちらにもいない（designer=推進側・lead=決定者・playtester=動作/難度・reviewer=コード品質。面白さの判定は出荷後のユーザー感想のみ＝3 作凡庸着地と今回の UX 詰まりの構造的真因）。候補 (a) 企画段階に「面白さ批評役 devil」（選定前に各案へ飽き/詰まり/単調/前作の轍を攻撃反証）(b) 製造後に「面白さヒューリスティック点検」（意思決定の幅・支配戦略の有無・テンポ・伝わりやすさを観点採点）。**限界**: エージェントは面白さそのものは感じられず、相関する構造の点検に留まる（最終判定は遊ぶ人）。スキル変更のため要ユーザー判断。
