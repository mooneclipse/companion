...
- **本番反映は OWNER 承認待ち（restart 未実施・push なし・commit は orc 側指示で）**: lead(orc) 統制ゲート。検証は別ポート47872 で実施し本番47825 には非接触（実装+自己動作確認+STATUS 記録まで）。playtester 実機検証と code-reviewer レビューを後段で通してからまとめてコミットする（orc 側が指示）。**未コミット**。
