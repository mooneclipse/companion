...
- [ ] pending 再評価のパス不一致バグ修正 — 共用 TODO **#100** (2026-07-13 発見)。保存側 (`ai_evaluator.save_pending_evaluation`) は CWD 相対 `pending_evaluations/` (systemd WorkingDirectory = repo ルート直下) に書くが、リトライ側 (`main._resolve_pending_dir`) は `Path(__file__).parent` アンカー = `python/youtube_checker/pending_evaluations/` (空) を見るため pending が永遠にリトライされない (サマリーの pending_retried 常時 0)。docstring の「cd するので一致」前提が現構成で不成立。解決を片側に寄せて統一 + 07-09 障害分の残存 19 件の回収確認まで
