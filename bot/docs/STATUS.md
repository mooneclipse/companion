...
- **反映**: bot はメモリ常駐のため**次回 service restart で本番投入** (検知網の追加のみで挙動変更なし、急がない)
