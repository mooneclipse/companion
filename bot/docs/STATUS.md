...
### #126 自発発話への返信の文脈断絶を解消 — 未読控え + 返信引用の prompt 前置き (2026-07-21、commit `4f1b5be`、restart 待ち)
...
- **デプロイ状態**: bot はメモリ常駐のため **restart 待ち (OWNER 承認要)**。restart 後の実機観察 = 次回 ephemeral 発話への返信が文脈通りに繋がるか / remind 文面に話題名が入るか / 非起票回が沈黙するか
