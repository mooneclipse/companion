...
**デプロイ**: `systemctl --user restart companion-bot` は auto mode classifier がブロック (本番デプロイは OWNER 名指し承認が要る判定、#110 timer enable と同型)。OWNER の restart 実施待ち — **restart までは旧コードが稼働中で #115 の修正は未反映** (今夜 19:11 の osekkai 号令自体は旧コードでも動作する、osekkai 経路は #110 で対処済みのため影響なし)。
