...
- **反映**: restart は呼び出し側で実施予定 (このエントリ時点では未反映)。以後の観察点: companion_thoughts.jsonl の観察行に「。」連結の自由記述が現れる頻度 (thought 申告の定着度)
