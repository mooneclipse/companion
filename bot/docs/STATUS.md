...
- **デプロイ**: bot.py はメモリ常駐のため **restart 待ち**。restart までは新規 `/tweet` 保存に `origin: external` が付かない (CLAUDE.md ルール側は次セッションから即有効)。restart は OWNER 名指し承認が要る運用のため未実施のまま報告。
