...
- 2026-07-22 OWNER から実際の bot 発話ログ・transcript を指摘され、4 点の改善案を実装 (すべて未起票の自発改善、コード修正)。
  - **改善案 #1**: `build_proactive_prompt` の話題ヒントが『さっき○○のこと考えてたんだけど』を silence_hours に関わらず渡していたため、10〜19 時間空いても「さっき」を連発する事故 (2026-07-21/22 実障害、OWNER 指摘)。固定例文をやめ、silence_hours (≤3h/それ以上/不明) に応じた言い回し判断基準を渡す形に修正。
  - **改善案 #3**: `interests.active_threads` (自発発話の話題滲ませ用) が `state=="researched"` の thread を interval 更新のたびに `last_touched` 延命させ、新しい話題を枠から押し出していた。非 researched を優先する安定ソート 2 段に変更 (`should_investigate`/`should_ticket`/`should_remind` の signal 選定には無影響)。commit `fa6c57c`。
  - **改善案 #4**: 「ほかに気になる話題は?」と深掘りされたとき、proactive prompt に渡した機械観察 (ytcheck 推薦本数・ディクテーション回数等) をそのまま報告口調で読み上げる事故を実観測。`PERSONA_SYSTEM_PROMPT` (全経路共通層) に「感想が無ければ正直に手持ちが無いと伝えてよい」を追記。commit `bba4a0f`。
  - **改善案 #2**: OWNER が bot の話し方の癖を訂正しても state 化されず別セッションで再発する問題。新規 `style_notes.py` (state = `sessions/companion_style_notes.json`、gitignore 対象、atomic write、`[[style-note: ...]]` marker で OWNER 訂正のみ記録) を追加。read-back (既存ルールを守らせるデータ) と emit-instruction (marker を出させる指示) を分離し、read-back は `build_proactive_prompt`/`compose_chat_prompt` という per-call prompt 本文層に展開 (`PERSONA_SYSTEM_PROMPT` のような system prompt 定数には入れない — 既存テストスイートが静的定数として検証している点と `run_claude` の `system_prompt` override が osekkai 専用の唯一の例外という制約を崩さないため)、emit-instruction は marker を剥がす `on_message` の既定 #chat 分岐にのみ持たせた (他経路に持たせると marker が Telegram 本文に漏れる)。設計方針は advisor 相談の上で着手前に本 STATUS.md に記録 (commit `1f641b2`)。commit `05d20f0`。
  - advisor 相談 (改善2の設計分離) → 着手前設計メモ commit `1f641b2` → 実装 commit `05d20f0`、code-reviewer 修正必須 0 (軽微 3、うち state 変更時の `logger.info` 追加 1 件を反映、STATUS.md 記録徹底 / 無関係な他サブプロジェクト変更を commit に巻き込まない、の 2 件は運用注意として本エントリに反映)。483→515 tests pass (interests 60、style_notes 13 新設、bot.py 側 style-note 関連 統合含む)。
  - **完了後 advisor 相談で追加ブロッカー発覚・修正**: `#chat` session は on_message 既定分岐と `_run_proactive` talk 分岐で共有 (同一 topic_key で `--resume`) されるため、on_message で出た `[[style-note: ...]]` が会話履歴に残り、talk 側の出力が `--resume` 履歴の自己強化でこの marker を再生成し Telegram に生 marker が漏れる穴があった (`[[next]]` の「marker 剥がしは toggle 非依存で常に行う」という既存不変条件に新 marker が追従していなかった)。talk 分岐で style-note を常に剥がし値は破棄する修正 + 2 marker (next-hours/style-note) の出現順不定に対応するループ化 + `_ProactiveTalkHarnessBase`/`ProactiveInvestigateTest`/`ProactiveTicketTest`/`ProactiveRemindTest` の `STYLE_NOTES_PATH` 隔離漏れ (talk フォールスルー時に実運用 state を読みに行く隠れた test isolation 漏れ) も修正。回帰テストは修正前に実際に fail することを確認済み。515→518 tests pass。commit `62e35d6`。
  - **restart 未実施**: 本改善はまだ本番 companion-bot.service に反映されていない (OWNER 明示承認と健全性確認が必要、過去の Done エントリと同じ運用)。次回セッションで OWNER 承認を得てから `systemctl --user restart companion-bot.service` を実施し、本エントリに実施時刻を追記すること。

### /say・/quota 未使用コマンド削除 (2026-07-21)

- **背景**: `/say` (TV 読み上げ) と `/quota` (予算 / 公式利用率表示) の Telegram slash command 自体が未使用と判明したため撤去。関連機構のうち他経路で現用のものは維持:
  - `/quota` 側: `budget_guard` (run_claude / 自発発話 / investigate / remind の全経路で現用) と `quota.py` モジュール本体 (`quota.JST` 等多数使用中) は削除せず。`cmd_quota` / `_fetch_official_usage` / `BotCommand("quota", ...)` / `CommandHandler("quota", ...)` を bot.py から削除
  - `/say` 側: `voice_command.cmd_say` / `MAX_SAY_TEXT` / `_say_lock` は自発発話の声載せ (`_dispatch_proactive_voice` → `_proactive_voice_worker`) が現用のため維持。`slash_say` / `BotCommand("say", ...)` / `CommandHandler("say", ...)` を bot.py から削除。`voice_command.append_ledger` (`voice_ledger.jsonl` 書き込み) は `/say` 専用だったため voice_command.py から削除、`VOICE_LEDGER_PATH` 定数も未参照化に伴い削除。voice_status.py の `/status` voice サマリから ledger 由来の「last /say」表示部分を撤去 (`voice/.state/last-result-*` 由来の OK/FAIL/padding skipped 集計は `/say` 非依存のため維持)
- **コメント整合**: `/say` を前提にしていたコメント (osekkai intent_store timeout の比喩元、proactive voice の `_say_lock` 直列化説明、`_proactive_voice_worker` の ledger 分離理由) を実態 (現在の唯一の呼び出し元は自発発話) に合わせて書き換え
- **テスト**: `FetchOfficialUsageTest` (5)・`AppendLedgerTest` (1)・voice_status の `test_last_say_from_ledger` (1) の計 7 件を削除、484→477 tests pass。commit `0161825`
- **デプロイ状態**: restart 済み (15:27、OWNER が AskUserQuestion で明示承認、active + slash commands 登録から say/quota 消滅 + notify socket listening を bot.log で確認、本番投入済み)。push は未実施。code-reviewer 修正必須 1 (設計メモの voice_ledger 現用記述残り) → commit `74f41df` で反映

### #126 自発発話への返信の文脈断絶を解消 — 未読控え + 返信引用の prompt 前置き (2026-07-21、commit `4f1b5be`、restart 済み・本番投入済み)

- **背景 (事実確認で確定)**: ephemeral 3 分岐 (investigate / ticket / remind) は使い捨て session で発話を生成し #chat へ投稿するだけで、会話 session の transcript に発話が残らない (last_prompt_at のみ更新、`読み取りツール使用ターンを会話 session に混ぜない` 意図の設計)。OWNER がその発話に返信すると、resume した会話 session は発話前トピックへの発言と解釈する。実障害 2 件を transcript で裏取り: 07-21 09:02 remind (AWS Route53 起点) への「なんの話？」に前話題 Stream Deck で応答、発話を全文引用されても「このスレッドでは出ていない」と自分の発話を否定 / 07-20 12:49「ナイス判断」(ticket 分岐報告への返し) に「特に何かあった?」と空振り (直前 /reset の新規 session も重なる)。talk 分岐は会話 session を resume するため正常に繋がる (07-18 シュタゲ実例) — 断絶は ephemeral 3 分岐に限定と確定
- **設計**: 発話送信後に未読控え (`sessions/pending/<topic>.json`、最大 3 件・1 件 500 字、atomic write) へ append し、次の on_message で pop (read+delete を 1 回で確定、claude 起動失敗時の書き戻しリトライは作らない = 補助情報でありエラーループ防止の 1 回確定原則) → `compose_chat_prompt` (純関数) が「あなたは少し前この一言を投稿している」内部メモとして OWNER メッセージに前置き。**即時のセッション注入案は不採用** (claude 起動が 1 回余計 + 出力破棄になる。前置き方式は追加起動ゼロで同効果)。Telegram 返信引用 (`quoted_reply_text`、forum topic root への自動 reply は message_id == thread_id で除外、400 字上限) も同前置きに載せる
- **様式強化 (圧縮報告の抑止)**: PROACTIVE_REMIND_PROMPT に「何の話題の振り返りかが分かる話題名を一言の中に必ず入れる」(07-21 実例は話題名なしの内部結論 69 字)、PROACTIVE_TICKET_PROMPT に「非起票の理由・経緯を本文に書かず [[thought]] 行のみへ」(07-20 実例は「今回は起票しない」判断説明 168 字をそのまま投稿 — 空で終える指示はあったが理由説明の禁止が明示されていなかった)
- **レビュー/テスト**: 461→484 tests pass (新設 22 = PendingContextTest 7 / ComposeChatPromptTest 6 / QuotedReplyTextTest 6 / ProactivePromptStyleTest 2 ほか remind 配線テストに控え検証、既存 Proactive*Test 3 クラスの `_PENDING_DIR` tmp 隔離で本番 sessions/ 非汚染)。code-reviewer 修正必須 0、軽微 1 (壊れた at timestamp で応答が 1 回落ちる → tolerant parse、+1 test で 484) を反映
- **デプロイ状態**: restart 実施済み (2026-07-21 11:41、OWNER が AskUserQuestion で明示承認)。健全性確認 = active / NRestarts=0 / slash commands 11 件登録 / notify socket listening (osekkai thread_id=1115) / 起動後 traceback なし。**本番投入済み**。実機観察の残り = 次回 ephemeral 発話への返信が文脈通りに繋がるか / remind 文面に話題名が入るか / 非起票回が沈黙するか

### #125 Stop hook ボイスの発生源申告 — claude -p 実行に COMPANION_VOICE_SOURCE=bot を付与 (2026-07-21、commit `db870ca`、restart 済み・本番投入済み)
...
- **変更**: `claude_runner.py _claude_env()` に `COMPANION_VOICE_SOURCE=bot` 1 行 (bot 系は osekkai / proactive 含め全経路がここを通る)。batch 側は `ytcheck/run.sh` / `maintenance/scripts/trends-weekly.sh` に export 追加。hook 3 本 (`~/.claude/hooks/complete-dispatch.sh` / `notify-dispatch.sh` / `error-dispatch.sh`、git 管理外) に SRC テーブル + 診断ログへ `src=` 追加。`~/.claude/sounds/renbot.wav` 新規 (VOICEVOX 玄野武宏 id=11 = 既存 hook 音のキャラ ren、同形式・同パディング、engine は一時起動→停止で復元。当初 voice-design.md v2.0 記載の四国めたん id=2 を誤流用し OWNER 指摘で再生成 2026-07-21。なお現用の話者は hook 音・/say とも玄野武宏 id=11 で統一済み (2026-06-12 persona 軸 2)、四国めたん id=2 は旧仮置きで現用なし — 同日の文書整理で voice-design.md 冒頭に正誤ヘッダ追加、memory `hook-voice-speakers` が正)
...
- **デプロイ状態**: ytcheck / trends は次回 timer 実行から有効 (プロセス常駐なし)。bot は restart 実施済み (2026-07-21 08:31、OWNER が AskUserQuestion で明示承認)。健全性確認 = active / NRestarts=0 / slash commands 11 件登録 / notify socket listening / 再起動後 traceback 0。**本番投入済み**
...
- **設計判断 (#118 で確定)**: **明示コマンド案を採用** — 自然文判定 (「今日は休む」発話の検出) は誤爆 (「休むか迷う」等) + 上位 CLAUDE.md の文言マッチ禁止原則に触れるため不採用。state 書き込みは claude セッションにさせない OWNER 裁定を維持し、既存 `_osekkai_intent_store_run` (intent_store.py subprocess) で bot Python 側完結。CommandHandler 経由なので on_message (`~filters.COMMAND`) には流れず、コマンド文字列が今夜の意図として誤記録されることもない
- **`/rest`**: `tonight-rest` を叩き休むフラグを設定 — trigger.py `_common_gate` が resting を参照するため当日の号令・振り返りが全沈黙する (D-8)。`/rest off` で解除。それ以外の引数は usage
- **`/backlog`**: `list` / `add <内容> [YYYY-MM-DD]` / `done <番号>` の 3 サブコマンド (要件 §3-3 の週次仕込み、従来 CLI のみ)。末尾トークンが実在日付かつ本文が残るときだけ締切として切り出す純関数 `parse_backlog_add_args` (迷ったら本文扱い = 誤解釈で本文を欠落させない側)。list 表示は intent_store CLI の人間可読出力をそのまま流す (整形ロジックを 2 箇所に持たない)
- **レビュー**: code-reviewer 修正必須 0、軽微 1 — 単一トークン `-` 始まり本文 (`/backlog add -x`) が argparse にオプション誤認され rc=2 → `--` セパレータで positional を確定して反映。同型の穴が既存 `tonight-intent` 呼び出し (OWNER 発話の意図記録) にもあったため同修正を適用
- **テスト**: 441→461 pass (新設 20)。実 CLI smoke (env override で scratchpad に隔離、実 data/ 不変) で全 argv 経路 + resting set/off 往復を確認
- **デプロイ**: restart 済み (2026-07-18 00:26、OWNER が AskUserQuestion で明示承認)。健全性確認 = active / NRestarts=0 / slash commands 11 件登録 (`rest`・`backlog` 含む、post_init ログで実測) / notify socket listening (osekkai thread_id=1115) / 起動後 traceback なし。**本番投入済み**
