...
- `quota.py` / `voice_status.py` → `.claude/rules/quota.md`（ledger append-only / 金額非記録 / voice_ledger は撤去済み）
- `voice_command.py` → `.claude/rules/voice.md`（engine on-demand / 成否 1 回判定 / 呼び出し元は自発発話のみ）
- `claude_runner.py` → `.claude/rules/claude-runner.md`（ClaudeOptions / to_cli_args / stderr 分類は表面化専用）

## git

- **`~/companion/` モノレポの 1 サブディレクトリ**（2026-07-11 切替、チケット #82。独立 repo・`companion-bot.git` remote は廃止）。repo root は `~/companion/`、remote は `github.com/mooneclipse/companion`。運用の正は上位 `~/companion/CLAUDE.md` git 運用方針。
- commit は**パス明示**（`git add <対象パス>`、`-A`/`.` 禁止）。push は claude が実行せず、commit 後に `cd ~/companion && git push origin main` を 1 行表示して OWNER が叩く。
- gitleaks pre-commit hook 必須（モノレポの hook 1 本で全域カバー）。
- commit メッセージは既存ログスタイル（`feat(bot):` / `fix(...)` / `docs(bot):` / `test(...)` = scope 付き）に揃える。Co-Authored-By trailer は付けない。
