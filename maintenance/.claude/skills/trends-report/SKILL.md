...
本スキルは project skill (`maintenance/.claude/skills/trends-report/`) のため、
`claude -p` の cwd が maintenance ルートでないと `/trends-report` が解決されず
Unknown command で無音失敗する。systemd 経由は `systemd/companion-trends.service` の
`WorkingDirectory=` で、shell 実行は `scripts/trends-weekly.sh` 内のサブシェル `cd` で
cwd を固定済み (2026-06-04 障害の再発防止。詳細は `docs/STATUS.md` 同日 entry)。
