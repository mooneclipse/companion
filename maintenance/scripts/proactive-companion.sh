...
#   7. 発火タイミングを通ったか — self-schedule (自己申告) が確率パスを置換する (チケット #92):
#      state の next_self_at (bot 側が talk 発話回の claude 申告から書く epoch) が
#      あれば、それが唯一の判定になる (未来なら skip = claude の意図した静けさを
#      乱数で破らない / 期限到来なら roll なしで発火)。申告は handoff 成功時の
#      書き戻しで消費する (1 申告 = 1 発火)。キー無し / parse 不能は従来の確率パス
#      (PROACTIVE_PROBABILITY を関心 index の活性度で変調した実効確率) に倒す。
#      ※確率パスの実効確率は「関心 index の活性度」に連動して上下する (固定→波)。
...
# パス定数は env override 可 (sandbox でのドライラン検証用。本番 = 未設定で既定パス)。
STATE_FILE="${PROACTIVE_STATE_FILE:-${HOME}/companion/maintenance/.state/proactive}"
OUR_LOG="${PROACTIVE_LOG_FILE:-${HOME}/companion/logs/maintenance/proactive-companion.log}"
...
MORNING_REPORT_FILE="${PROACTIVE_MORNING_REPORT_FILE:-${HOME}/companion/dashboard/.state/morning-report.json}"

SOCK="${PROACTIVE_SOCK:-${XDG_RUNTIME_DIR:-/run/user/$(id -u)}/companion-bot.sock}"
...
INTERESTS_INDEX="${PROACTIVE_INTERESTS_INDEX:-${HOME}/companion/bot/sessions/companion_interests.json}"
BOT_DIR="${PROACTIVE_BOT_DIR:-${HOME}/companion/bot}"
...
# next_self_at=<epoch、次回発話タイミングの自己申告 (チケット #92)。bot 側が talk
#   発話回の claude 申告から書く唯一のキー。step7 で 1 回引いて確率ロールを置換し、
#   handoff 成功時の書き戻しで意図的に書かない = 消費する>)。
...
# --- 7. 発火タイミング (self-schedule 自己申告 → 無ければ確率パス) -----------------
# チケット #92: 前回の talk 発話回に claude 自身が申告した「次に話したくなる時刻」
# (state の next_self_at = bot 側が書く epoch) があれば、確率ロールを **置換** する:
#   - now < next_self_at → 発火しない (claude の意図した静けさを乱数で破らない)
#   - now >= next_self_at → 発火する (roll なし。gate 1〜6 はすべて通過済みなので
#     沈黙閾値・週上限・snooze の安全弁はそのまま効く)
# 申告は 1 回で消費する (handoff 成功時の書き戻しで next_self_at を書かない = 次の
# 申告は次の発話回に claude が改めて出す。handoff 失敗 / 手前 gate skip の回は state
# 無変更なので申告は持ち越される)。キー無し / parse 不能 (非整数) は従来の確率パスに
# 倒す (bootstrap-safe: bot 側 toggle off や申告なし回の翌 tick は従来挙動)。判定は
# state 1 read で確定、乱数は増やさない (2 周目ルール派生原則と両立)。
next_self_at=$(state_get next_self_at)
if [[ "$next_self_at" =~ ^[0-9]+$ ]]; then
    if (( now_epoch < next_self_at )); then
        log "skip: self-schedule not due (next_self_at=$next_self_at, now=$now_epoch)"
        exit 0
    fi
    # 期限到来 → roll を引かず発火。最終ログ用の値は self-schedule 経路と分かる表記にする。
    roll="-"
    eff_probability="self-schedule(next_self_at=$next_self_at)"
else

# --- 7b. 確率パス (関心 index 活性度で変調した実効確率) ---------------------------
...
fi  # step7 self-schedule / 確率パス分岐の終端 (どちらの経路もここに来たら発火確定)
...
if printf '%s' "$message" | nc -U -N "$SOCK"; then
    ...
    # 注意: ここはキー明示列挙で書き戻す (bot 側 _write_state_values の総なめ保持と
    # 非対称)。state に新キーを足すときは、この書き戻しにも必ず足すこと。
    # 例外: next_self_at は **意図的に書かない** = 自己申告の消費 (チケット #92、
    # 1 申告 = 1 発火。次の申告は bot 側が次の talk 発話回に改めて書く)。
