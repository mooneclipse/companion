...
⏸ 5 は **ユーザー帰宅待ち** (2026-06-10): バッテリー取り外し済みのため再起動すると POST の「Battery not detected」警告で停止し、F1 の物理押下が要る。リモート (`sudo reboot` のみ) では完遂できない。急ぎではない。物理操作のついでに BIOS の警告スキップ設定 (Dell なら「Warnings and Errors」→ Continue 系) の有無を確認すると、以後の無人再起動・停電復帰が可能になる (S6-6 の電源断耐性とも関連)。
...
- memory 34 エントリ (索引と実ファイル一致) は参照先全て現存、削除ゼロ。settings.json は変更なし (棚卸しのみ)。S6-5 (cleanupPeriodDays) はオーケストレータ側で別途扱う
...
- 作業項目:
  ...
  7. **BIOS の無人起動設定** (2026-06-10 追加、S1-5 で判明): バッテリー未検出警告で POST が停止し F1 物理押下が要る = リモート再起動・停電復帰が不可。BIOS に警告スキップ (Dell「Warnings and Errors」→ Continue 系) と AC Recovery (通電で自動起動) があれば両方有効化したい。確認・変更は物理操作時 (S1-5 の再起動時が好機)
...
- [ ] S2 未使用サービス・パッケージ整理
- [ ] S3 ディスク・ログ衛生
- [x] S4 claude 設定・スキル・CLAUDE.md 品質レビュー — 完了 (2026-06-10)。S6-5 (transcript 自動掃除 / cleanupPeriodDays) はオーケストレータ側で別途
