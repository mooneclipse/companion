...
- [ ] S6-6 ノート PC 延命チューニング (省電力・発熱、バッテリーレス AC 直結前提) — **計測フェーズ完了 (2026-06-29)**。sudo 不要分を先行: CPU 48°C/ファン正常/schedutil 適切/DPMS 消灯なし(要設定)/SQLite 不使用(電源断リスク低)/thermald 稼働。残 = powertop + 省電力適用 + DPMS 設定 + BIOS 確認 + 物理清掃 (全て要 sudo or 物理操作)
