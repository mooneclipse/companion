...
- [x] S6-2 オフディスクバックアップ追加 — 完了 (2026-06-12)。restic + KIOXIA USB 64GB (FAT32)、初回 snapshot 成功・復元検証済み (結果は S6-2 セクション)
- [-] S6-6 ノート PC 延命チューニング (省電力・発熱、バッテリーレス AC 直結前提) — **powertop 完了 (2026-06-29)**。①計測: CPU 48°C(idle)/52-55°C(負荷時)/ファン正常/schedutil 適切/SQLite不使用(電源断リスク低)/thermald 稼働。②powertop: `--auto-tune` 26項目適用(i915 runtime PM/SATA/PCI/NMI watchdog off/VM writeback)、`powertop-autotune.service` (oneshot) で恒久化。③CPU governor: schedutil で適切(C7 88-92%滞在)、変更不要。④DPMS: **スキップ** — TV は YouTube 等で常用するため DPMS 全体消灯は不可、LVDS-1 単独 `xrandr --off` はダッシュボードの `--kiosk-monitor=1` 番号が変わるリスクがありユーザー判断で見送り。⑥電源断耐性: SQLite 不使用確認済み。残 = ⑤物理清掃 + ⑦BIOS 設定 (次の再起動・物理操作時)
