...
const driverSrc = `
  ...
  // 登りは act(0,-1) の連打だけで機能する(行き先セルに既にはしごがあるため moveTo 後の
  // applyGravity 冒頭チェックで即 return=留まる)。テストの player-state 注入方針(G.dug 注入と
  // 同じ)に倣い、実際に通過したマスへ都度はしごを直接注入する(playerTrail はフォロー中の女の子が
  // 居ない間は末尾1点へ畳まれる=v0.11.0 GC の仕様のため、事後の一括注入では経路が失われて使えない
  // と判明=当初案からの見直し)。
  function markLadderHere() { if (G.py > 0) G.placedLadders.add(G.px + "," + G.py); }
  ...
  function step(dc, dr) {
    ...
    // 掘り抜き直後の重力落下で 1 手が複数マス進むことがある(中間マスを act 外から個別に見る
    // 手段が無い)ため、開始 by〜終了 G.py の縦区間まるごとへはしごを敷く(通過点の漏れ防止)。
    // 開始列(bx)と終了列(G.px)が違う(横移動)場合は開始マスにも敷く(そこへ戻る経路として)。
  ...
  // 自機を地表(row0)へ「実際に climb して」戻す: step/digTowards が掘り進む先々ではしごを敷いて
  // いるので、真上が空間の限りそのまま act(0,-1) の連打で登れる(=着地後は重力無効で留まる)。
  // 塞がっていれば足跡を辿って横へ逃げてから登る(縦坑先掘りに頼らない実 climb)。
  function climbToSurface() {
    let guard = 0;
    while (G.screen === "dive" && G.py > 0 && guard < 400) {
      guard++;
      const bx = G.px, by = G.py;
      // 真上が空間なら登る(step/digTowards が掘り進む際に経路へ敷設済みのはしごを使う。ここで
      // 新たに敷いても行き先=真上には効かないので敷かない)。
...
const followA = await page.evaluate((driver) => {
  ...
  // v0.20.0 判断C: digTowards が掘り進む際に経路へ敷設済みのはしご(step 内 markLadderHere)を
  // 使って登る(ここは rows 記録のためループを手書きしている=climbToSurface と同じ流儀)。
  G.monsters = [];
  // climb 中も足跡追従が走るよう、この act が moveTo を呼ぶ。
