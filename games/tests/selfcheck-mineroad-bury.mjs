...
//     静かに除去 = EXP/ドロップ 0)
//  C. 活動範囲(v0.18.0 判断 E ③で活動箱 MONSTER_ACTIVE_RANGE(16) へ統一。圏外は hp/bt/buried
//     とも不変 = 衰弱も脱出もしない。圏外構成は孤独な山 80×80 の盤で行う)
...
// ============================================================================
// 2. 活動範囲ゲート(v0.18.0 判断 E ③で書き換え): 旧 BURIED_WAKE_RANGE(4) は撤去され、土中 tick は
//    monstersAct の活動箱 MONSTER_ACTIVE_RANGE(16) で律速される。裏庭 15×15 は全域が箱内になる
//    (自機がどこに居ても Chebyshev ≤ 15)ため、圏外ケースは孤独な山(id6、80×80)の盤で構成する。
//    しきい値は CONST から読む(数値の見直しに機械追随)。despawn(両軸 ±28 AND)に届かない位置を選ぶ。
// ============================================================================
console.log("\n== 2. 圏外 tick 停止(活動箱 16、盤=孤独な山) ==");

const range = await page.evaluate((helpers) => {
  eval(helpers);
  G.dungeonId = 6; // 孤独な山 80×80: 活動箱 16 の圏外が構成できる盤。
  startDive();
  const wake = CONST.MONSTER_ACTIVE_RANGE;
  const desp = CONST.MONSTER_DESPAWN_RANGE;
  // 自機(0,0)から圏外(>wake)だが despawn(両軸 >desp の AND)には届かない埋没個体を 1 体選ぶ。
  const b = pickBuried((m) => {
    const dx = Math.abs(m.col), dy = Math.abs(m.row);
    return Math.max(dx, dy) > wake && !(dx > desp && dy > desp);
  });
  ...
  const out = {
    found: true, wake, dist: Math.max(Math.abs(b.col - 0), Math.abs(b.row - 0)),
    before, after: { hp: b.hp, bt: b.bt || 0 },
    stillBuried: b.buried === true, noDug: !G.dug.has(fkey(b.col, b.row)),
    stillListed: G.monsters.indexOf(b) >= 0,
  };
  G.dungeonId = 0; // 後続セクションは裏庭で startDive し直す。
  return out;
}, HELPERS);
check("2a 圏外個体は 5 tick 後も hp/bt/buried 不変・脱出なし(despawn 圏内=除去もされない)",
  range.found && range.dist > range.wake && range.after.hp === range.before.hp && range.after.bt === range.before.bt &&
  range.stillBuried && range.noDug && range.stillListed, range);
