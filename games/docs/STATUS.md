...
- **follow-up（次増分候補）**: AI/活動範囲の原作化（bfsStep 完全追跡 → 原作 signum 直線 + 確率ゲート + 活動 16 マス圏 + despawn 28。**実機で過圧ならキュー先頭へ**）/ ランタイムスポーン（キュー 3 位。埋没人口の補充源として本機構とセットで効く）/ selfcheck-bury 10c の golden 値化 / 土中死・崩落再埋没の実プレイ発現は深ダンジョンの帯構成で自然に出るため状況待ち。
