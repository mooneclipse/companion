...
### v0.14.0（2026-07-15 着手、アイテム拡充＝原作 item.csv 45 種への拡張 + アンテナ保険の第11増分＝§2 バックログ 7 番（最終項）、チケット #56、**着手時 翻案判断 先記録**・実装は sonnet implementer 委任（OWNER 指示））

OWNER 発注＝チケット #56（`mineroad-remake-plan.md` §2 バックログ **7 番＝アイテム拡充（原作45種への拡張、§8）+ アンテナ保険（電波範囲内は力尽きてもアイテムをロストしない §8）**）。設計正本は原作 item.csv / craft.csv / 仕様まとめ §8。**忠実再現・決定論（ランタイム乱数禁止）・ワールドレイヤー（tileType/girlPositions/oreAt 系列/monster/hazard/avalanche の初期生成）非介入・playtester + code-reviewer 通過 → commit（2026-07-06 裁定ゲート）** を厳守。

- **CSV→実装 翻案の各判断（着手前に記録）**:
  - **A. 鉱石・素材体系を原作実名へ引き直す（v0.4.0 独自 4 種体系の解消、2 周目でなく一段引く）**: v0.4.0 の銅/鉄/金/ダイヤ原石 4 種と独自レシピ（石ツルハシ←銅3 等）は当時「verbatim」と記録されたが原作 item.csv/craft.csv に存在しない翻案体系。この上に 45 種カタログを重ねると素材が 2 本立ちになり後で必ず引き直しになるため、本増分の土台として**鉱石を原作 6 種（石炭/鉄鉱石/化石/鋼/ルビー/ダイヤ）へ名寄せ**し、`oreAt` の深度帯マッピングを 6 種へ拡張（浅=石炭 → 深いほど鉄鉱石/化石/鋼/ルビー/ダイヤ、dungeon.csv の鉱石ドロップ分布と v0.13.0 per-dungeon×per-band 構造に乗せる）。クラフトは craft.csv 実物を参照して実レシピへ差し替え（ツルハシ=鋼2+骨1+石炭1 / アンテナ=鉄鉱石3+鋼2+化石1 等）。**ツルハシ 4 段 power ゲートは掘削ゲートの核として維持**（原作は単一ツルハシ+確率破損だが、確率破損は決定論違反のため採らない=v0.4.0 判断維持。段ごとの材料は craft.csv に段が無いため翻案でスケールし、選定理由を実装時に本エントリへ追記）。v0.8.0 商人 4 行の対価も名寄せに追随。
  - **B. 45 種カタログのデータ化 + dead-item 非陳列（v0.8.0 哲学踏襲）**: tiles.js に `ITEM_DATA` 全 45 種を item.csv 忠実書き起こし（ID/名前/タイプ/HP/SP 回復量/最大所持数/用途タイプ。数値は verbatim、説明文は自前要約=著作権境界）。**取得経路を開く種**（機能する忠実サブセット）= 鉱石 6 種（oreAt）/ ドロップ素材（骨・動物の皮・クモの糸・種火・動物の血・生肉・龍のウロコ=monster.csv ドロップ表の表示のみだった `G.drops` を実アイテム化）/ 飲食（生肉・焼き肉・動物の血・キノコ・夢キノコ・フルーツ=BAG から使用、HP/SP 回復は item.csv verbatim・eff 上限で頭打ち）/ 道具（ハシゴ・ツルハシ・アンテナ）。**取得経路を開かない種**（カタログ定義のみ、クラフト/商人/ドロップに陳列しない）= 爆弾・バケツ 3 種・矢・マジックハンド・フックショット・木の杭・天井フック・ロープ・浮島・海綿・タネ 3 種・かかし・レンガ・堅土・赤土・パンツ・幸運のお守り・解毒薬（毒未実装）・作業台・鉄骨片・鉄骨ブロック — いずれも新規メカ（投擲/汲取/放水/土設置/杭/育成/霊/毒）が要り 1 増分を超える。次増分候補として残す。**焼き肉の変化則**（生肉所持でマグマ接触→焼き肉化=item.csv MAGMA 変化列 verbatim）は状態遷移のみで決定論を壊さないため採用候補、実装可否は implementer 判断で記録。**回復薬（原作に無い v0.4.0 独自）は廃し飲食系で置換**（45 種外のアイテムを並走させない=体系 1 本化と同根。potions は永続対象外のため save 互換影響なし）。インベントリの置き場所は散在 state（ore/drops/mushrooms/…）を 45 種キーの単一 `G.items` へ集約する方向で implementer が設計。
  - **C. アンテナを原作忠実の設置型 + 保険へ引き直し**: 現行「所持フラグで女の子全域透視」（v0.4.0 翻案）を廃し、**設置型**（空間マスに設置、1 ダンジョン上限 20 本=仕様まとめ §8、回収可）へ。**電波範囲** = 地表(row0)から基本深度 R0 + 「自身が電波圏内にある」設置アンテナが半径 R を中継（連結で範囲拡張=グラフ伝播、player 操作由来の state のみで決定論）。R0/R の実数値は原作から取れないため盤面規模（15〜80 列）を踏まえ implementer が翻案し本エントリへ追記。**保険（本丸）** = 力尽き時に自機が電波圏内なら携行アイテムを retry へ持ち越し、圏外なら従来どおり全ロスト。v0.12.0「消耗品は力尽きロスト=原作の緊張設計」とは矛盾しない（原作自身がアンテナを保険として提供しており、保険の追加こそが忠実）。設置済みアンテナは fail を跨いで残す（固定 seed 同一盤面で位置が意味を保つ=保険の実用性の核）、クリアで消去。**透視は電波圏内の女の子表示に限定し、fog/探索率（seen）には非介入**（探索率 100% クリア条件の意味を保つ。全域透視→圏内限定は難度が原作方向へ戻る変更として明記）。
  - **D. 検証**: selfcheck-items（新規、45 種カタログ整合/飲食使用/名寄せ後クラフト/アンテナ設置・連結・圏判定/保険 carry/圏外全ロスト）+ debug gate AD 追加。既存 selfcheck 3 本（save/companion/grow）+ debug gate A〜AC(29) 回帰 ALL PASS 必須（鉱石名寄せで gate Q3/B 系の期待値追随が要る場合はテスト緑化でなく実装整合を確認してから追随し、変更点を記録）。決定論（Math.random/Date.now 実呼び 0）・既存 6 作 URL 不変・短高 viewport はみ出し 0・本番 47825 非接触（別ポートで検証・PID 直 kill）。
- **実装完了（2026-07-15、sonnet implementer、VERSION=v0.13.1→v0.14.0）**:
  - **台帳ギャップの発見（着手前）**: 本エントリ着手時点で `app.js` は既に **VERSION=v0.13.1**（はしご設置/回収/上掘り＝`placedLadders`）まで実装済みだったが、本 STATUS には v0.13.1 のエントリが無かった（`git log` 上は 1 コミットに集約された履歴のため経緯不明）。本増分はそのギャップを埋めず（台帳追記は親/OWNER 領分）、**v0.14.0 の差分だけを本エントリに記録**する。同じ理由で `debug-mineroad.mjs` の gate 番号は既に **AD が v0.13.1 はしごで使用済み**だったため、判断D「gate AD 追加」の指示を **gate AE** へ読み替えて追加（意図との差分）。
  - **判断A 実装値（鉱石名寄せ）**: `ORE`/`ORE_META` を COPPER/IRON/GOLD/DIAMOND(4種) → **COAL/IRON_ORE/FOSSIL/STEEL/RUBY/DIAMOND(6種、item.csv 売値 150/200/400/3000/1200/15000 verbatim)** へ全面差し替え。`DUNGEON_BANDS` の `oreW`（全9ダンジョン×70帯）は `dungeon.csv` 列21-26（石炭/鉄鉱石/化石/鋼/ルビー/ダイヤ）を使い捨て Python スクリプトで機械抽出し出典照合可能な形で流し込み（手打ち転記なし、負値1件＝孤独な山 floorTo76/80 の石炭列 `-1/-4` は原作 CSV 側の生値のため重み0にクランプ）。`monW`（モンスター）は不変・非改変。`oreAt` は6種の累積しきい値抽選へ拡張。**暗黙翻案の記録（code-reviewer 指摘）**: `dungeon.csv` の鉱石ブロックは列27-29に骨/タネ/赤いタネの重みも持つ（浅帯で骨は約5%）が、本実装は6鉱石(列21-26)のみを `oreAt` の抽選対象とし、骨/タネ/赤いタネの重みは無視して6種のみで暗黙に再正規化している。骨は monster ドロップ経由（BAT 20%）に入手経路を譲る翻案（原作は採掘からも骨が出たが、本実装では出ない＝骨がツルハシ/はしごのクラフト素材として重要になった v0.14.0 では、入手経路が原作より狭まっている）。タネ/赤いタネは判断Bの dead-item（栽培メカ未実装）のため、oreAt からは意図的に対象外。**craft.csv 実レシピ採用**: 石のツルハシ=石炭3（翻案・単一素材の安価な初段）/ 鉄のツルハシ=鋼2+骨1+石炭1（craft.csv id27 verbatim）/ **ダイヤのツルハシ=鋼4+化石2+ダイヤ1+骨2**（鉄段レシピを2倍スケール+ダイヤ原石を追加、原作に無い最上段のための翻案、選定理由=名前に沿わせダイヤ原石を素材に含めた）/ はしご=鉄鉱石1+石炭1+骨2（craft.csv id2 verbatim）/ アンテナ=鉄鉱石3+鋼2+化石1（craft.csv id3 verbatim）。CRAFT_RECIPES/SHOP_RECIPES の cost 形を `{ore:{...},item:{...}}` へ統一し、判定(`canTrade`)・支払い(`payCost`)・産物付与(`grantResult`)・コスト表示(`costText`)をクラフト/商人で共通化（旧 costText/tradeCostText・recipeRedundant/tradeRedundant の重複実装を統合、対症療法ではなく1回の一本化）。HUD インベントリ帯は鉱石 span を4→6（`.inventory` は既存 `flex-wrap:wrap` で吸収、追加 CSS 変更なし）。
  - **判断B 実装値（45種カタログ + 飲食置換）**: `tiles.js` に `ITEM_DATA`（45件、id/name/hp/sp/max/open/note、item.csv verbatim + 自前要約）。open=20種／dead=25種（内訳は判断Bの列挙どおり、解毒薬・海綿は v0.5.0 から既存のモンスタードロップ経路自体は変更していないが本増分では「用途未実装」の dead 扱いに据え置き＝非改変）。工房オーバーレイに**第5タブ「アイテム」を新設**（読み取り専用 BAG ビュー、dead 行は `.item-dead` で暗く非インタラクティブ表示、45行の操作UIは作らない＝advisor 助言どおり軽量案）。`.craft-tabs` は5タブ化に伴い `flex-wrap:wrap` + `flex-basis:30%` へ調整（3タブ/2タブの2段組、はみ出し0を全 viewport で確認）。**回復薬（v0.4.0 独自）を廃止**し `G.potions`/`btn-potion`/`CONST.POTION_HEAL` を削除、item.csv 実在の飲食（フルーツ/夢キノコ=既存回帰、**焼き肉=新規**HP40・**動物の血=新規**HP30、いずれも item.csv verbatim・`effHpMax()` で頭打ち）へ置換。個別の `usePotion`/`useFruit`/`useDreamMushroom` 相当の重複実装を作らず、所持元（G の専用フィールド or `G.drops`）の違いを吸収する汎用 `useConsumable(id)` + `CONSUMABLES` テーブルへ1本化（既存呼び出し名 `useFruit`/`useDreamMushroom` は薄いラッパーとして温存しテスト互換を保った）。**焼き肉の変化則を採用**: `cookMeatInMagma()` をマグマ中の行動フック（既存 `hazardHpChip` 発火点）に接続し、所持する生肉を1個ずつ焼き肉へ変換（決定論・状態遷移のみ、生肉は生の状態では HP0=食料としては無価値＝マグマで焼く動機付けとして機能）。商人に焼き肉行（鋼2、shop.csv verbatim）を追加。
  - **判断C 実装値（アンテナ設置型 + 電波網 + 保険）**: `G.antenna`(bool) を `G.antennaItems`(所持数) + `G.placedAntennas`(Set、設置済み座標) へ引き直し、`placeAntenna()`/`recoverAntenna()` を `placeLadder`/`recoverLadder` と同型で実装（isSpace かつ地中でのみ、1ダンジョン上限 `CONST.ANTENNA_MAX=20`＝仕様まとめ §8 verbatim）。**電波網 R0/R の翻案値**: 全9ダンジョンは深度(`DEPTH_ROWS`)が15〜99と大きく異なるため固定値は不適（小規模で過大／大規模で過小）と判断し、**深度比例**（`ANTENNA_R0 = max(3, round(DEPTH_ROWS*0.2))` / `ANTENNA_R = max(4, round(DEPTH_ROWS*0.28))`、直線距離=Euclidean）を採用。裏庭(15層)は R0=3/R=4、オンカロ(99層)は R0=20/R=28。連結判定は「地表基礎範囲の縁から半径 ANTENNA_R 以内、または既に連結した別アンテナから半径 ANTENNA_R 以内」の不動点計算（アンテナ数上限20のため O(n²) で軽量、乱数・ハッシュ不使用＝判断Cの「player 操作由来の state のみ」を満たす）。**実装中に検出した設計バグ1件**: 初版は「基礎範囲の**内側**に物理的に立つアンテナだけが連結」という条件で実装したため、基礎範囲のすぐ外（本来は中継届く距離）に置いたアンテナが自分自身の位置すら圏内にできない不具合が selfcheck で検出された（`row<=R0` の直接比較 vs 「基礎範囲の縁までの距離が R 以内」の違い）→ 縁までの垂直距離 `distToBaseZone(row)` で判定する形へ修正し再検証。**透視**を `G.antenna` 全域参照から `inRadioCoverage(col,row)` へ差し替え（fog/探索率=seen には非介入、判断Cの明記どおり難度が原作方向へ戻る）。**保険は2系統に分離**（advisor 助言）: (a) 設置済みアンテナ位置は fail を無条件で跨ぎクリアで消去＝`mineroad_antennas_N` キー（v0.12.0 `mineroad_save_N` とは別、スキーマ非改変）、(b) 携行アイテム（ore/drops/mushrooms/dreamMushrooms/fruits/roastMeat/ladders/antennaItems/pick）は死亡時に電波圏内だった場合のみ `mineroad_insurance_N` キーへ1回だけ保存し次回 `startDive()` で消費（読んだら即削除＝二重持ち越し不可）、圏外なら何もせず既存の「ランごと全リセット」がそのまま効く（v0.12.0「消耗品ロスト=緊張設計」と非衝突）。
  - **検証**: `selfcheck-mineroad-items.mjs` 新規43アサート・**3回連続 ALL PASS**（45種カタログ整合/飲食4種のHP回復+頭打ち/生肉マグマ変化/名寄せ後クラフト5レシピ/名寄せ後商人5レシピ/アンテナ設置・回収・20本上限・電波網連結/保険carryと1回消費/圏外全ロスト回帰/アンテナ位置のfail跨ぎ・クリア消去/決定論3回一致/非介入）。`debug-mineroad.mjs` に **gate AE**（アイテムタブ画面操作・クラフト5レシピ画面操作・アンテナ設置画面操作・透視条件・保険の画面操作往復）を追加、既存 gate A〜AD(30) 全回帰 + 新設 AE、**3回連続 ALL PASS**。既存 `selfcheck-mineroad-save.mjs`/`selfcheck-mineroad-companion.mjs`/`selfcheck-mineroad-grow.mjs` は3回連続 ALL PASS（save gate は電波圏内での fail だと保険が発火し「ランごとリセット」の前提が崩れるため、圏外まで潜ってから力尽きさせるようテスト側を追随＝テスト緑化でなく仕様の正しい理解に合わせた改修、変更点は本エントリに記録のとおり）。決定論（Math.random/Date.now 実呼び0）・既存6作URL不変・短高viewport(412×680/730)はみ出し0・本番47825非接触（別ポート47860、`server/app.py` を GAMES_PORT=47860 で自前起動→検証後 PID kill）を維持。
  - **本番反映**: 静的ファイルは live-from-disk（2026-07-06 OWNER 裁定、`server/app.py` restart 不要）。ゲートは「playtester + code-reviewer 通過 → commit」（restart 不要な既存ファイル編集のため、review 通過後の commit 時点で実質反映）。
- **code-reviewer（2026-07-15、修正必須なし・軽微4件）**: 数値の verbatim 主張を実 CSV と機械突合＝oreW 全70帯・ITEM_DATA 45種・craft/shop レシピすべて不一致0。保険の3キー分離（save_N/antennas_N/insurance_N）と read-and-delete 1回消費、クリア時 insurance 残留経路なし、決定論・非介入・回復薬残骸ゼロを確認。軽微4件のうち3件を commit 前に反映＝①ANTENNA_MAX=20 の出典ラベル訂正（item.csv でなく仕様まとめ §8。app.js コメント3箇所+本エントリ）②oreW 暗黙翻案（骨/タネ/赤いタネ列の無視）の記録追加（判断A実装値に反映済み）③`tests/probe-merchant-flow.mjs` の btn-potion 残骸クラッシュ→v0.14.0 追随修正（削除でなく修正＝実マウスタップ page.mouse の独立検証手段として存続。追随中に **v0.13.0 以来の既存不具合**＝タイトルがダンジョン選択制になって以降 `#ov-action` タップでは dive へ遷移できず probe が実質死んでいた、も発見し `.dungeon-btn` タップへ修正、PROBE RESULT: PASS 確認）。
- **playtester 独立検証（2026-07-15、全6項目 PASS）**: gate A〜AE 3回連続 ALL PASS（27 gate、pageerror 0）+ selfcheck 4本 ALL PASS に加え、**gate AE とは別実装の独立ヒットテスト 25/25 × 3回連続**（全入力 page.mouse 画面座標 + elementFromPoint 最前面 assert、canvas 直接 dispatch なし）＝45行カタログの名前順序一致 / HUD 鉱石6種 / 実採掘の oreAt 決定論クロスチェック（事前スキャン予測と実加算一致）/ アンテナのクラフト→深所孤立設置は圏外（連結バグの回帰負例）→row7 設置で連結圏内（正例、裏庭実測 R0=3/R=4）/ 短高 412×680/730 はみ出し0。既存4作+mineroad+healthz 全200、決定論静的検査0、本番47825非接触（検証 PID 直 kill 後に本番 healthz 200 生存確認）。headless の限界（実ブラウザツールバーの svh 挙動は再現不可）は明記のうえ実機確認は OWNER に委ねる。
- **follow-up（次増分・次回テスト編集時）**: ①電波網の多段中継（アンテナ経由の先のアンテナが連結）と孤立アンテナ非連結の実アサートが selfcheck/gate AE に無い（グラフ伝播は本増分の核、次増分までに補強推奨＝code-reviewer 軽微4件目）②debug 総合出力ラベル「(F) 既存 6 作 回帰」は実体4作とのラベル乖離（コスメティック）③shop.csv「キノコ100↔夢キノコ1」の交換向きは原作 CSV 列順から見ると逆の可能性（v0.8.0 出荷済み挙動のため本増分では非変更、所見のみ）④龍のウロコ・種火はカタログ実装済みだが現行6種モンスターサブセットでは実入手経路なし（ボスモンスター増分で開通）。
- **進捗**: **完了（2026-07-15）**。実装（sonnet implementer 委任）→ playtester 独立検証 PASS → code-reviewer 修正必須なし（軽微3件反映済み）→ commit。live-from-disk のため commit 時点で本番反映（2026-07-06 裁定ゲート充足）。§2 バックログは 7 番まで全消化＝残りは dead-item 25種の新規メカ増分・ボスモンスター・電波網テスト補強など次期バックログ候補として OWNER 発注待ち。

### 原作挙動監査（#119、2026-07-18 完了。「仕様書全体が Opus 4.8 のハルシネーションで汚染されている気がする」の OWNER 疑義への全数監査）

- **体制**: researcher 6 並列（①移動・重力 / ④モンスター AI / ⑤女の子 / ⑥なだれ / ⑦力尽き / 旧仕様まとめ全文監査）+ リメイク実装抽出 1（app.js/tiles.js 全読）。全報告の load-bearing な主張（SP 変換式・水中溺れ式・ランタイムスポーン・救出判定・女の子消滅・なだれ崩落コード・アンテナ上限/連結・復活処理）はメインセッションが該当行を開いて一次確認。researcher 間の解釈衝突 1 件（タイル ID 56-95 の意味）は一次確認で決着（= 土系固体。dungeon.csv 定義行と一致）
- **成果物**: `docs/mineroad-original-spec.md`（原作挙動の正本。信頼度 3 値注記付き、jadx 復元欠落の未特定箇所も明示）+ `docs/mineroad-diff-ledger.md`（差分台帳: 忠実/翻案/誤解由来/未実装 × 深刻度 + 修正発注キュー 6 件）。旧 `~/mineroad-analysis/MINE_ROAD_仕様まとめ.md` 冒頭に監査結果と正本への誘導を追記（正本昇格の明文化）
- **監査の結論（OWNER 疑義への回答）**: 全面汚染ではない。CSV・逐語系（ダンジョン表/45 アイテム/20 モンスター/レシピ/交換レート/クリア条件/UI/再生成ペナルティ）は全数突合でほぼ 100% 一致。**確定誤りは動的メカニクスの因果記述 4 件に偏在** = 既知 3 件（上掘り不可 / 水が動かない / 埋没モンスター機構）+ 新規 1 件（**水/マグマは「スタミナ」でなく体力(HP)を消耗** — tutorial.csv 明記 + `cm.java:1146-1169` で二重確認）。教訓 = データ転記は信頼可、挙動因果はコード裏取り必須
- **主要な新規判明（正本 spec 参照）**: 原作は SP1000/HP100 で SP 不足分の 1/2 が HP 化・地下のみ移動 SP2 / 水中は SWIM 値ターンまで無傷→超過後 HP 直撃 + 浮力（重力無効）/ ランタイムスポーン（行動ごと 30%、16〜28 マス圏、y≧11）と 28 マス超 despawn / 女の子は毎ターン経路探索追従・ロストで完全消滅・プロフィール自動生成 / なだれは 3 段階弱化→1 マス崩落→連鎖伝播（上 100%・左右 30%）+ 埋没は「土中に居る」継続ダメージ（自機 maxHP/10/ターン）+ 水食でなだれ土が自然発生 / 力尽きは 3 択で同一ダンジョン続行の地上復活（アンテナ圏内で全ロスト回避、上限 20・地表起点 BFS 連結 10.0f）
- **修正発注キュー（差分台帳の深刻度順）**: 1. 水/マグマ機構総取り替え（誤解由来・高、#119 発注枠②） 2. 埋没モンスター機構替え（誤解由来・高、発注枠③） 3. ランタイムスポーン導入（中） 4. なだれ機構替え（中） 5. 女の子ロストの重み（中） 6. 低群（SP 経済・乱数幅・特殊個体ほか）。発注は本台帳から増分単位で
- **進捗**: 完了（2026-07-18）。docs のみ・コード非改変・本番非接触

### v0.15.0（2026-07-18 着手、掘削 8 方向の原作合わせ＝OWNER 実機 FB「上・斜めが掘れず探索しにくい」→「ちゃんと原作にあわせて」の直発注、**着手時 翻案判断 先記録**）

OWNER 実機 FB 3 点（①上掘り・斜め掘り不可 ②モンスターが土から発生 ③水が動かない）を受け、原作 APK デコンパイル（`~/mineroad-analysis/out/sources/jp/windbellrrr/app/minerroad/`、jadx・ProGuard 難読化だがロジック可読）を researcher 3 並列 + 本セッション一次確認で裏取り。**本増分は①のみ**（1 セッション 1 増分）。②③は下記「原作仕様の裏取り記録」に確定内容を残し次増分候補へ。

- **原作の掘削方向仕様（コード確定）**:
  - `bc.java:210-244` exchangeTapToDirection: dx/dy が独立に -1/0/+1 の **8 方向**を別コードへ変換（画面 5 分割象限 or 隣接 8 マス直タップ）。
  - `cm.java:349-407` 上方向ハンドラ: 対象タイルが `ae()`（ツルハシ power で掘削可）なら共通掘削エンジン `n.h()`（`n.java:2250`→`a(int,int,int,true)`）を呼ぶ。**真上（iB=0）の付帯条件 `w(i,i2)` は自機セル自身の通行判定＝常に真 → 真上掘りは power ゲートのみで無条件**。はしご等の前提なし（researcher の「垂直貫通不可」解釈は誤りと一次確認で訂正）。
  - 斜め上（iB≠0）: 横隣 `w(i+iB,i2)` または真上 `w(i,i2-1)` が空間のとき可（完全な壁中への斜めねじ込みのみ不可）。
  - `cm.java:205-242` 下方向ハンドラ: 斜め下は dx/dy 同時更新の移動 + 同エンジンでの掘削。上移動（ジャンプ）は真上が空間のとき 1 マスのみ。
- **翻案判断（着手前に記録）**:
  - **A. 入力**: `pointerEnd` の隣接判定を Manhattan 1 → **Chebyshev 1（隣接 8 マス）**へ。原作の画面 5 分割象限入力は不採用（遠隔タップの誤爆と既存「タップしたマスへ動く」慣習の破壊を避ける）。方向ボタン UI は 4 方向のまま（斜めはタップで）。
  - **B. 真上掘り**: v0.13.1 の**はしご前提ゲートを撤去**し power ゲートのみで掘削可（原作合わせ、OWNER「意図していない仕様」明言）。掘り抜き時に自機は移動しない（原作は掘削と移動が別アクション。既存クライムで次タップ登り）。はしご設置/回収メカ自体は非改変で温存（原作にも存在するアイテム。用途再定義は次増分）。
  - **C. 斜め**: 斜め上＝（横隣 or 真上が空間）を前提条件に、対象が空間なら重力ありの moveTo（原作ジャンプの落下と同型＝足場があれば階段登りが成立）、掘削可なら掘る（掘り抜きで移動しない）。斜め下＝対象が空間なら重力ありの moveTo、掘削なら既存下掘り同様に掘り抜きで前進。斜め下の前提条件は上と鏡映（横隣 or 真下が空間）— 原作 `h(n)` の細部は jadx 復元欠落で完全確定できず**「前提」として明示的な翻案**。
  - **D. bump-to-attack を 8 方向へ拡張**（原作は移動＝攻撃が同一方向系）。女の子 snake 追従は足跡履歴（`G.playerTrail`）ベースのため斜め足跡も自然に消化＝非介入。世界生成レイヤー（tileType/girlPositions/oreAt/monster/hazard/avalanche/mushroom）全て非介入。
  - **E. 既知の残差分（意図的に踏み込まない）**: 原作は重力 + ジャンプ 1 マスで垂直縦坑の素登り不可（階段掘り・はしごが正攻法）だが、リメイクは v0.1.0 以来「掘った縦坑を 1 行動 1 マスで素登り」＝原作より緩い。真上掘り解禁で縦エレベーター（真上掘り→クライム反復）が可能になり撤退緊張が下がるリスクがあるが、重力モデルの全面原作合わせは中核作り直し級のため本増分では踏み込まず **OWNER 実機の手触りで判断**（悪ければ次増分で登坂条件を原作へ寄せる）。
  - **F. 検証**: selfcheck-dig8（新規: 真上掘り無はしご可/権限 power ゲート維持/斜め上前提条件の正負/階段登り成立/斜め下移動・掘削/8 方向 bump 攻撃/決定論 3 回一致/非介入）+ debug gate **AF**。v0.13.1 の「はしご無し上掘り不可」を assert する既存テストは**仕様変更の明示的追随**として書き換え（テスト緑化でなく本エントリを根拠に）。既存 selfcheck 4 本 + debug gate 全回帰・決定論（Math.random/Date.now 実呼び 0）・既存作 URL 不変・短高 viewport はみ出し 0・本番 47825 非接触（別ポート検証・PID 直 kill）。
- **原作仕様の裏取り記録（②③、次増分候補・OWNER 発注待ち）**:
  - **② モンスターと土**: 原作に「プレイヤーが掘った瞬間の新規スポーン」は**無い**（掘削処理 `n.aa()` にスポーン呼び出しなし）。実体は 2 つ＝(a) マップ生成 `q.java:349-385` で空間セルへ生成した個体が **WORM（ID11）なら直後にセルを土 58 で上書き＝生成時に埋める**、(b) 土中の個体は毎ティック `bk.java:140-151` で HP を削られつつ `PER_BROKEN_SOIL`%（monster.csv「埋没掘りper」、WORM/MOLE/SOIL DRAGON/GHOST=100）で自分の STR により覆い土を破壊して**自力で出てくる**。リメイク v0.5.0 の「掘った瞬間にそこへ抽選スポーン」（trySpawnBuryMonster）は「埋没掘りper」の翻案としては原作と機構が異なる（結果の見た目は近いが、原作は決定論的に「最初からそこに居る」+自律脱出）。
  - **③ 水**: 原作の水（タイル 24〜31 の 8 段階密度）・マグマ（32〜39）は**毎ターン左・右・下へ流れるセルオートマトン**（`n.java:751` P() → `n.java:1851` b()、空マスへ最低密度 24/32 を生成＝流れ込み、同系合流で密度 +1、31/39 で満水）。バケツ汲み取り・海綿・埋め立ての水量操作系とも整合。リメイク v0.6.0 の固定オーバーレイ `hazardAt` は「動かない水」＝原作と明確に異なる。P() の毎ターン駆動元のみ jadx 復元欠落で未特定（流動ロジック自体は実コード確認済み）。
- **実装値（2026-07-18、implementer 委任、VERSION=v0.14.0→v0.15.0）**:
  - **判断A 実装値（入力）**: `pointerEnd` の隣接判定を Manhattan 1 → Chebyshev 1 へ（`|dc|>1 or |dr|>1` は無反応、自マス `(0,0)` は除外）。方向ボタン UI は 4 方向のまま非改変。
  - **判断B 実装値（真上掘り + act 一本化）**: v0.13.1 のはしご前提ゲート（`placedLadders.has` 分岐＝上掘り専用に掘削コードが丸ごと複製されていた）を撤去し、`act()` を全 8 方向共通の単一経路「モンスター→攻撃 / 斜め前提条件 / isSpace→移動 / 固体→power ゲート→掘削」へ一本化（複製解消はリファクタであり挙動追加ではない）。上方向（dr=-1、真上/斜め上）の掘り抜きは自機非移動＝既存の埋没スポーン時「前進しない」分岐に相乗り（revealAround/monstersAct/renderHud/checkFail）。真上クライム（空間への 0,-1）は従来どおり noGravity で非改変。はしご設置/回収（placeLadder/recoverLadder/描画）は非改変で温存。**なだれ土の崩落タイミング**: 上掘りで markUnstableDug されたマスの崩落解決は次の moveTo の resolveCaveins に委ねる＝埋没スポーン時と同じ遅延解決で、v0.13.1 はしご版「掘り抜き後に登った時点で崩れる」と等価（新規分岐を足さない）。
  - **判断C 実装値（斜め）**: 前提条件は「横隣 `(px+dc,py)` or 縦隣 `(px,py+dr)` が `isSpace`」の 2 判定のみ、不成立は行動消費なしの完全無反応。斜め上の空間へは重力あり moveTo（足場あり＝階段登り成立・足場なし＝落ち戻り＝原作ジャンプ同型、selfcheck 2e/2f で両方確認）。斜め下は空間→重力あり moveTo、掘削→掘り抜きで前進（既存の下掘りと同じ costPaid moveTo）。SURFACE への移動は `isSpace(row<=0)=true` で自然に包含（v0.4.0「SURFACE を power ゲートに落とすと地表を歩けない」退行の回避を isSpace 1 点に集約）。
  - **判断D 実装値（bump 攻撃の順序、implementer 選択）**: bump-to-attack は**斜めの前提条件より先**に判定＝完全な壁中でも隣接 8 マスのモンスターへ攻撃できる。理由＝原作の入力観「隣接タップ＝その方向へのアクション」+ 遊びやすさ（囲まれた際に反撃できない理不尽の回避）。selfcheck 4a で「横隣・真上・斜め先が全固体の壁中から斜め攻撃成立・移動なし・掘りなし」を確認。女の子 snake 追従（playerTrail）は斜め足跡を自然に消化し破綻なし（既存 gate C/N/P 回帰 PASS で確認、追従コード非改変）。
  - **テスト追随（テスト緑化と区別して記録）**: v0.13.1「はしご無し上掘り不可」を負例 assert する既存テストは**存在しなかった**（gate AD ははしご設置/回収と「はしご有りで上掘り可」の正例のみ＝ゲート撤去後も同一 assert のまま PASS、書き換え不要。funproxy の「登れない→break」判定も掘り抜き非移動のため挙動不変）。実際の追随は VERSION assert の機械追随のみ＝selfcheck 4 本（save/companion/grow/items）+ debug gate A の `v0.14.0`→`v0.15.0`（挙動 assert の書き換えゼロ）。
  - **検証（判断F 充足）**: `selfcheck-mineroad-dig8.mjs` 新規 24 アサート（真上掘り無はしご可 + power ゲート維持 + 掘り抜き非移動 + クライム不変 / 斜め上前提条件の正負 + 階段登り + ジャンプ落ち戻り / 斜め下の負例・掘削前進・斜め移動対称性 / 壁中斜め bump 攻撃 / 地表からの斜め上無反応 / はしごメカ温存 / 8 方向操作列の決定論 3 回一致 / girlPositions・tileType 非介入、ポート 47876）**3 回連続 ALL PASS**。`debug-mineroad.mjs` に **gate AF**（画面座標ヒットテスト: はしご無し上掘り→非移動→クライム / 斜めタップ掘削→階段登り / Chebyshev 距離 2 タップ無反応。カメラ lerp 整定待ちを挟み描画座標とタップ座標を一致させる）を追加し、既存全 gate + AF **3 回連続 ALL PASS**。既存 selfcheck 4 本 **3 回連続 ALL PASS**。決定論静的検査（Math.random/Date.now/performance.now 実呼び 0、gate I）・既存 4 作 URL 不変（gate F + 手動 curl で `/`・`/tomoshibi/`・`/nagori/`・`/akari/`・`/healthz` 全 200）・短高 viewport 412×680/730 はみ出し 0（gate E2）・本番 47825 非接触（GAMES_PORT=47860 自前起動→PID 直 kill→本番 healthz 200 生存確認）。
- **code-reviewer（2026-07-18、修正必須なし・軽微2件は follow-up 送り）**: 判断 A〜F の実装整合（Chebyshev 1 + (0,0) 除外 / 真上掘り power ゲートのみ・掘り抜き非移動・noGravity クライムは dc===0 限定 / 前提条件の鏡映翻案 / bump 先行判定）、act() 一本化での既存分岐（GIRL/鉱石/キノコ/なだれ/埋没スポーン/SURFACE/境界）の全方向維持、決定論・世界生成非介入・snake 追従非破綻を確認。selfcheck-dig8 24/24 はレビュアー側でも実走確認。gate AD3 が掘り抜き非移動でも PASS する構造（py 変化まで最大 10 タップ→次タップのクライムで変化）も実コードで裏取り。
- **playtester 独立検証（2026-07-18、3 回連続 全 9 項目 PASS・fail 0）**: gate AF とは別実装の固定経路 約30手 実タップ検証（`tests/verify-mineroad-dig8.mjs` 新設、全手 page.mouse + elementFromPoint 最前面 assert、canvas 直接 dispatch なし）。斜め掘り/斜め移動+重力落下/距離2 無反応/はしご無し真上掘り（SOIL 1 タップ・HARD 2 タップ・SP HUD 一致・非移動・placedLadders=0 維持）/クライム/階段登り 1 段上昇/斜め上前提条件の負例（完全無反応）/power 不足×表示（WOOD vs HARD・STONE vs ROCK の tier 境界 2 段）/決定論（fresh コンテキスト 2 回×3、16 スナップショット完全一致）/既存 5 URL+healthz 全 200/短高 412×680・730 はみ出し 0。pageerror 0。本番 47825 非接触（別ポート 47862→PID 直 kill→本番 healthz 200 生存確認）。headless の svh 限界は明記のうえ実機最終確認は OWNER 委ね。
- **follow-up（次回テスト編集時・次増分）**: ① pointerEnd の (0,0) 除外と |dr|>1 枝の負例テストが未整備（`act(0,0)` 直叩きは自マス moveTo で 1 行動消費するが UI 経路では到達不能=実害なし、reviewer 軽微 2 件）② 真上掘りで GIRL を発見した場合「自機が女の子セルに入らないまま発見」する初のケースになり、初回 advance で女の子が Chebyshev 2 をホップして見える可能性（視覚のみ・機構は無傷、実機で気になったら対処）③ 判断 E の縦エレベーター（真上掘り→クライム反復）による撤退緊張低下は OWNER 実機の手触り判断待ち。
- **進捗**: **完了（2026-07-18）**。実装（implementer 委任）→ playtester 独立検証 PASS → code-reviewer 修正必須なし → commit。live-from-disk のため commit 時点で本番反映（2026-07-06 裁定ゲート充足）。②水流動・③埋没モンスターの原作化は監査チケット #119（v0.15.0 完了後・別セッション）の差分台帳から発注。

### v0.16.0（2026-07-18 着手、水/マグマ機構の原作合わせ＝差分台帳キュー 1 位（誤解由来・高）、チケット #120、**着手時 翻案判断 先記録**）

差分台帳（`mineroad-diff-ledger.md` ③）の修正発注キュー 1 位。旧仕様まとめの「水/マグマはスタミナを激しく消耗」取り違え（正 = 体力(HP)。tutorial.csv 明記 + `cm.java:1146-1169` 一次確認、正本 spec §4）が v0.6.0 実装に直輸入されていたのを総取り替えする。**固定オーバーレイ hazardAt + SP×3/×4 割増 + マグマ HP chip 2 → ①流動セルオートマトン ②浸水は SWIM 値ターンまで無傷→超過後 HP 直撃 ③マグマ HP−maxHP/5 ④浮力（水/マグマ中は重力無効）**。1 セッション 1 増分（差分台帳キュー 2 位以降=埋没モンスター/ランタイムスポーン等は対象外。SP 経済の原作合わせ＝地表無料/地下 SP2/不足分 1/2 変換も低群のため触らない）。

- **原作仕様 → web 実装への翻案判断（着手前に記録）**:
  - **A. 流動の実体 = ランタイム流体 state（`G.fluid` Map: "col,row" → {kind, d:1..8}）+ 初期湧出 + 掘り当て湧出**。決定論ハッシュ `hazardAt`（tiles.js）は**流体の初期配置・掘り当て抽選として温存**（位相 +1597/+2389/+7919・ハッシュ消費系列は完全非介入 = 既存 gate W1/W2/W6 の決定論 assert を保つ）。startDive 時に「初期空間（isSpace）かつ hazardAt≠NONE」のマスへ密度 8（満水）で配置。**掘り抜いたマスが hazardAt≠NONE なら流体が湧出**（密度 8、`G.fluidReleased` Set で 1 マス 1 回限り=再掘での無限湧き防止）。**理由**: リメイクの世界生成は空洞率 5〜30% と低く、初期空間のみの播種では dungeon.csv 由来の帯別 hazardRate の遭遇率が 1 桁落ちる。「掘り当てて水脈が噴き出す」は旧実装（掘ると水が現れる）の遭遇率を保ったまま流動を足す翻案。原作は生成時に水タイルが空洞に置かれる（掘り当て湧出は無い）ため、ここは**忠実でなく遭遇率保存を優先した明示的翻案**。
  - **B. セルオートマトン規則（毎ターン・決定論）**: スナップショット走査（row 昇順→col 昇順）、そのターンに新規生成されたセルは同ターン処理しない=**毎ターン 1 マスずつ広がる**（原作の落下 1 マス/ターンの律に同調）。各流体セル: 真下が空間なら「流体なし→密度 1 生成 / 同種で密度<8→+1（合流増密）/ 異種→何もしない」。真下が固体 or 満水同種 or 異種なら左右それぞれへ同じ規則で展開。**非保存**（源は減らない。正本 spec §4「空マスへ最低密度 24/32 を生成・合流で増密・31/39 で満水」の読みどおり=水面下の連結空間が徐々に満ちる）。row 0（地表）へは生成しない。**翻案注記**: 原作 P() の駆動元・走査範囲は jadx 復元欠落（spec で ？）のため、走査順・「下が塞がっているときのみ左右展開」・水/マグマの相互不干渉は翻案。
  - **C. 浸水ダメージ = HP 直撃（SP 割増の全廃）**: `WATER_SP_MULT`/`MAGMA_SP_MULT`/`MAGMA_HP_CHIP`/`SWIM_MITIGATION`（v0.6.0）を撤去し、spendAction は素の SP_PER_ACTION に戻す（移動コスト側の割増は原作に無い= spec §4）。**水**: 滞在ターンカウンタ `G.breath`（原作 c[34] 相当）を水中で毎ターン +1、**SWIM 値を超えるまで無ダメージ**、超過後は毎ターン **HP − clamp(4 − perLv(SWIM), 1, 4) を直接減算**（SP を経由しない=原作 HP 直撃。takeDamage の SP カスケードは通さない）。水から出るとカウンタ 0（息継ぎ、`cm.java:1082-1084`）。SWIM 値 = `SWIM_BREATH_BASE(5) + perLv(SWIM) × SWIM_BREATH_PER_LV(5)`。**原作式 clamp(4−SWIM/5,1,4) の SWIM/5 を per レベル 0..4 に直写像**（Lv×5 = 原作 SWIM 値 0..20 相当。効きは「無傷ターン延長 + 超過後減額」の減算系=乗算軽減でない、spec §4 どおり）。原作 SWIM 初期値は未特定（c[33] 初期代入が jadx 欠落）→ 基礎 5 ターンは「掘り抜き水没から 1 往復戻れる猶予」の翻案。ダメージ実値 1..4 はスケール非換算で持ち込む（原作 HP100 → リメイク HP30。裏庭スケールでは息 5 + HP30/4 ≒ 最大 12 ターン水中生存=撤退判断が成立する緊張。リメイクはダンジョンも浅く滞在が短いため相対的に辛い方へ倒す）。**マグマ**: 猶予なし毎ターン **HP − ceil(effHpMax()/5) 直接減算**（原作 maxHP/5、5 ターンで死= spec §4 `cm.java:1224-1226`）。v0.14.0 の生肉→焼き肉変換（cookMeatInMagma）はマグマ滞在ターンへ移設（行動時→滞在ターンで等価）。**PER_SWIM の意味替え**: 「消耗倍率を除算」→「息 +5 ターン/Lv + 溺れダメージ −1/Lv」。per.SWIM の保存形式（レベル整数）は不変=セーブ互換。
  - **D. 浮力 = 水/マグマ中は重力無効**（spec §2/§4 `n.y()`）: applyGravity は自機の現在マスに流体があれば落下させず、落下ループ中も流体セルへ入った時点で停止（着水して浮く）。上移動は既存クライム（0,-1 noGravity）と斜め上 moveTo がそのまま泳ぎとして機能（新規分岐を足さない）。**女の子は非介入**（v0.11.0 足跡追従で独立重力なし。原作の GIRL 地形ダメージ=ロストの重み付けは差分台帳 5 位の別増分へ送る残差分）。モンスターも水と不干渉（出現地形フラグ/水中挙動は別増分）。
  - **E. ターン解決順**: 3 箇所の `monstersAct()` 呼び出し（moveTo / act 非前進経路 / attackMonster）を `resolveTurn()` 1 本に集約 = fluidStep()（流動）→ tickSubmersion()（浸水判定 + noteHazardEntry 移設 + マグマ調理）→ monstersAct()。順序 = 流動→浸水（**水が自分へ流れ込んだターンから浸水扱い**。原作の駆動順は未特定→翻案）。noteHazardEntry を moveTo から移すのは「静止していても水が流れ込んでくる」動的化に伴う責務移動。**崩落との相互作用**: 崩落土塊が着地したマスの流体は消す（原作の埋め立て=土で水を消すと整合する翻案）。不安定土の支持判定は isSpace のまま（原作 ad() も水/マグマ=支持なしで一致、spec §6）。
  - **F. 視覚・UI（新規アセット無し=URL/allowlist 不変）**: render の hazardAt 直読みを G.fluid 読みへ差し替え、密度で alpha 変調（浅い水は薄く・満水は濃く）。水没中は自機頭上に残り息ターンを canvas 直描画（DOM 追加なし）。息切れ時にヒント 1 回 + cueWater/cueMagma 文言を HP 直撃仕様へ更新。泡=空気ポケット（原作 g()==16、踏むと息リセット）は未実装→次増分候補として記録。
  - **G. 検証**: `selfcheck-mineroad-water.mjs` 新規（流動: 下→左右・増密・満水 cap・毎ターン 1 マス拡張 / 掘り当て湧出 1 回限り / 息: SWIM 値ターン無傷→超過後 HP 直撃・SP 不変・息継ぎリセット / SWIM Lv で延長+減額 / マグマ maxHP/5 猶予なし / 浮力: 水中重力無効・落下は着水停止 / 崩落の埋め立て / 同一操作列の決定論 2 回一致 / tileType/girlPositions/oreAt/hazardAt 非介入）+ debug gate 追加。**既存テストの明示的追随（テスト緑化と区別して本エントリを根拠に書き換え）**: debug gate W3（水 SP 割増）/W4（マグマ HP chip）と selfcheck-grow の hazardSpMult assert は旧 v0.6.0 仕様の assert のため新仕様（HP 直撃・SWIM 息延長）へ書き換え。W1/W2/W6（hazardAt 決定論・深度ゲート・非介入）は hazardAt 温存により不変。fun/funproxy は挙動変化で結果が変わりうるため再計測して honest 記録。既存作 URL 不変・短高 viewport はみ出し 0・本番 47825 非接触（別ポート検証・PID 直 kill）。
- **実装（implementer 自己動作確認、2026-07-18）**: 判断 A〜G を verbatim 実装（VERSION v0.16.0。app.js: CONST 差し替え=SWIM_BREATH_BASE 5/SWIM_BREATH_PER_LV 5/DROWN_DMG_BASE 4/MAGMA_HP_DIV 5、G.fluid/G.fluidReleased/G.breath/G.drownNoted、seedFluids/releaseFluidAt/fluidStep/tickSubmersion/resolveTurn/fluidAt/swimTurns/drownDamage 新設、spendAction 素の SP_PER_ACTION 化、applyGravity 浮力、resolveCaveins 埋め立て、render の G.fluid 読み+密度 alpha+残り息 canvas 直描画、cueWater/cueMagma 更新+cueDrown 新設。tiles.js: PER_DEFS SWIM 文言と PER_GAIN.SWIM_PER_LV 撤去のみ=ハッシュ非介入）。実装値の細部: 湧出は act の掘り抜き成立時 trySpawnBuryMonster 直後（湧出したターンの resolveTurn でその流体も流動源になる=CA の「同ターン非処理」は fluidStep 内生成セルのみに適用）/ 息切れヒントは G.drownNoted（水から出るとリセット）。**検証**: selfcheck-mineroad-water 新規 28 アサート（ポート 47877）**3 回連続 ALL PASS**、既存 selfcheck 5 本（save 22/companion 16/grow 13/items 43/dig8 24）各 ALL PASS、debug-mineroad 全 gate（A〜AF + W3/W4 新仕様）**3 回連続 ALL PASS**。fun/funproxy 実測（vs HEAD=v0.15.0 ベースライン同条件比較）: 全体力尽き率 50%→**25%**（optimal 5/5 fail→0/5 fail・dive 継続=旧 SP×3/×4 割増が慎重 bot を削り殺していたのが、息 5 ターン猶予で生存側へ。reckless は 5/5 fail 不変）、funproxy は到達 15/救出 1/撤退 0 不変・行動数中央値 76→68。数値ノブの再調整はしていない（honest 記録）。本番 47825 非接触（検証 47879/47881 自前起動→PID kill→本番 healthz 200 確認）。playtester 独立検証・code-reviewer・commit はメインセッション後段。
- **playtester 独立検証（2026-07-18、3 回連続 ALL PASS・fail 0・各 run 70 アサート）**: `tests/verify-mineroad-water.mjs` 新設（selfcheck/debug gate とは別実装。seed 41027 裏庭の実盤面から事前導出した固定経路、全手 page.mouse + elementFromPoint 最前面 assert、canvas 直接 dispatch なし）。実測 = 掘り当て湧出（hazardAt(8,12)=WATER 掘り抜き→G.fluid d8 + 画面ピクセル青優勢化）/ 流動（毎ターン 1 マス・d1→増密→d8 cap→満水後に左右展開）/ 溺れ（breath1..5 は HP30 不変・SP のみ 1/ターン、6 ターン目 HP30→26 を SP82 残存でも直撃、離水で breath0・再突入で無傷=息継ぎ）/ 浮力（落下は水面停止・水中横移動で吸い込まれない）/ マグマ HP−6 猶予なし 3 ターン連続（裏庭 magmaFrac=0 のため G.fluid 直接注入の補助検証と明記）/ 残り息の頭上描画（水没中白青・息切れ赤・離水/帰還で消滅）/ 決定論（fresh 2 回×3 run、22 スナップショット完全一致）/ 既存 5 URL+healthz 200・短高 412×680/730 はみ出し 0・pageerror 0。本番 47825 非接触（GAMES_PORT=47893 自前起動→PID 直 kill→本番 healthz 200 生存確認）。headless の svh 限界は明記のうえ実機最終手触りは OWNER 委ね。
- **code-reviewer（2026-07-18、修正必須なし・コミット可）**: 判断 A〜G verbatim 実装の突合、決定論（fluidStep スナップショット row→col 昇順ソート実在・乱数/時刻実呼び 0・湧出 1 回限り）、非介入（tiles.js 差分はラベル/未使用定数撤去のみ、girlPositions verbatim 一致）、checkFail 冪等・surfaceReturn の breath リセット・G.fluid 非永続の安全性（保険はアイテムのみ）を確認。selfcheck-water 28/28 はレビュアー側でも実走確認。軽微 3 件 = ①掘削途中タップ（remain>0）は resolveTurn を通らずマグマ/息が刻まれない（既存ターン意味論と整合・HEAD と同一構造で退行ではないと playtester も独立確認。本行をもって STATUS 明記とする）②旧 app.js + 新 tiles.js のキャッシュ版すれ違いで水中 stamina NaN の理論経路（SW 未登録で低確率、将来 `?v=` クエリ付与を検討=follow-up 送り）③撤去済み SWIM_MITIGATION の残留コメント 3 箇所（次回編集時に掃除）。
- **follow-up（次増分候補）**: 泡=空気ポケット（原作 g()==16、踏むと息リセット）/ 女の子・モンスターの水挙動（原作は GIRL も地形ダメージ=差分台帳 5 位と束ねる）/ index.html の `?v=` キャッシュバスティング / 残留コメント掃除。バケツ/海綿等の水量操作アイテムは原作に実在（spec §4）だが ITEM_DATA 非陳列のまま=別増分。
- **進捗**: **完了（2026-07-18）**。翻案判断 先記録 → implementer 委任 → playtester 独立検証 3 回 ALL PASS → code-reviewer 修正必須なし → commit。live-from-disk のため commit 時点で本番反映（2026-07-06 裁定ゲート充足、v0.15.0 と同じ扱い）。差分台帳キュー 2 位（埋没モンスター機構替え）以降は #119 台帳から別セッションで発注。

### v0.17.0（2026-07-18 着手、埋没モンスター機構の原作合わせ＝差分台帳キュー 2 位（誤解由来・高）、チケット #121、**着手時 翻案判断 先記録**）

差分台帳（`mineroad-diff-ledger.md` ④）の修正発注キュー 2 位。旧仕様まとめの「埋没モンスター = 掘削時スポーン」誤解（正 = 生成時に土中へ配置 + 自力脱出。`q.java:349-385` / `bk.java:140-151` 一次確認、正本 spec §5）が v0.5.0 実装（trySpawnBuryMonster = 掘り抜いた瞬間そのマスへ抽選スポーン）に直輸入されていたのを機構替えする。**掘削時抽選スポーン → ①生成時（startDive）に土中へ配置=最初からそこに居る ②土中の個体は毎ターン HP−1 ③bury%（PER_BROKEN_SOIL 本来の意味）で覆い土を自力破壊して脱出**。1 セッション 1 増分（ランタイムスポーン=キュー 3 位、なだれ機構替え=キュー 4 位、女の子の土中ダメージ=キュー 5 位はスコープ外）。

- **原作仕様 → web 実装への翻案判断（着手前に記録）**:
  - **A. 埋没の実体 = 生成時配置（q.java:349-385）**: trySpawnBuryMonster（掘削時抽選）を廃し、startDive で全固体マス（tileType が SOIL/HARD/ROCK）を走査、`buryMonsterAt(col,row,seed)` 非 null のマスへ埋没個体（buried=true）を配置する。**buryMonsterAt 自体（BURY_PRESENCE_RATE 0.16 × 帯別重み × bury% ゲート、位相 +233/+617/+3001・+557/+271/+6173・+1019/+643/+6173）はハッシュ・分布とも不変で温存** = 現行の遭遇率・配置分布を保存したまま、実体化タイミングだけ「掘った瞬間」→「生成時」へ動かす（v0.16.0 判断 A の hazardAt 温存と同型）。**翻案注記**: 原作の埋没配置は WORM 特例（空間生成→土 58 上書き）+ 出現地形フラグ土の種（△列順推定）で、PER_BROKEN_SOIL は配置に関与しないが、リメイクは遭遇率保存を優先して bury% 込みの既存テーブルを配置に使い続ける（bury% の二重役割 = 配置密度への重み + 脱出抽選、明示的翻案）。fail/retry の startDive 再列挙は決定論で同一配置を再現（「最初からそこに居る」が力尽きを跨いで成立）。
  - **B. 土中 tick（bk.java:140-151）**: 埋没個体は毎ターン **HP−1**（土中で衰弱=脱出時には削れている）+ **bury% で覆い土を自力破壊して脱出**（脱出 = 自マスの空間化 + アクティブ化）。HP 0 は土中死 = 静かに除去（EXP/ドロップ/演出なし=自機が関与していない撃破ではない）。**前提として明示**: 原作は共通掘削エンジンで STR により覆い土を削る多段破壊の可能性があるが、土の耐久値が一次未確定（jadx 復元欠落）のため、spec §5 正本の記述粒度どおり**単発成功 = 脱出**とする（WORM/SPIDER の bury100 は「近づくと即座に出てくる」= OWNER 実機 FB②「モンスターが土から出てくる」の原体験と整合）。
  - **C. 活動範囲ゲート（by.java:189 の限定導入）= 小マップで機構が壊れないための核**: 土中 tick は**自機から Chebyshev 距離 BURIED_WAKE_RANGE（=8）以内の個体のみ**。原作の「プレイヤーから 16 マス以内の個体のみ毎ターン処理」を埋没個体に限り導入する。これが無いと裏庭 15×15 ではダイブ開始直後に全埋没個体が一斉に衰弱・脱出し、「近づくと土の中が動き出す」原作の緊張が「開始数ターンの一斉孵化」に化ける。値 8 = 原作 16 の半分スケール（リメイクは HP 100→30・裏庭 15×15 とおおむね原作の半分規模）かつ VISIBLE_RADIUS の外 = 見えない場所から土の崩れる気配が近づく。圏外の個体は衰弱も脱出もしない。**アクティブ個体の全ターン行動（差分台帳④「活動/despawn」翻案・低）はスコープ外で不変**。
  - **D. 決定論（チケット #121 名指しの整合点）**: 脱出抽選はランタイム乱数でなく、**個体ごとの覚醒 tick カウンタ bt（圏内 tick のたび +1）**を使い `hash3(origCol+P1, origRow+P2, seed+P3+bt×P4) < bury/100` で確定（位相 P1..P4 は既存全系列と非衝突の新値を実装時に採番・記録）。同一 seed + 同一操作列 → bt 系列一致 → 脱出タイミング・土中死タイミングとも完全再現（Math.random/Date.now 実呼び 0 の既存方針を維持）。
  - **E. 世界への作用 = プレイヤー掘削と同型（採取なし）**: 脱出の空間化は G.dug.add + G.fallen.delete +（なだれ土なら）markUnstableDug + releaseFluidAt（掘り当て湧出=水脈マスを破ったら流体が湧く、v0.16.0 判断 A と整合）をプレイヤー掘削と同順で通す（原作はモンスターの土破壊も同一掘削エンジン n.h()、spec §3）。ただし collectOre/collectMushroom は呼ばない = 埋没個体が破った土の採取物は失われる（翻案注記）。G.dug 入りにより脱出跡は fog 越しに可視化されるが、「土が崩れた場所は分かる」演出として許容（別 state セットを増やさない）。**掘り当て**: 自機が埋没個体のマスを掘り抜いたら即アクティブ化（既存「埋没スポーン=前進しない」分岐と演出を流用=手触り不変）。**崩落との相互作用**: resolveCaveins で崩落土塊が着地したマスのアクティブ個体は buried へ戻す（原作「崩落で埋まった個体は土中個体化」spec §6。現行は土中に取り残されたまま行動し続ける既存エッジの解消）。
  - **F. 参照境界**: buried 個体は bump 攻撃・移動ブロック（monsterAt）の対象外 + 描画対象外（土の中は見えない=テレグラフなしも原作どおり）。脱出演出は既存 trySpawnBuryMonster の popup/SFX/ヒントを移設（未 seen セルでの脱出は演出を控えるかは implementer 裁量で記録）。tick 処理は monstersAct 内（monsterStep 冒頭の buried 分岐）= resolveTurn の解決順（fluidStep→tickSubmersion→…→monstersAct）不変。女の子の土中ダメージ・ランタイムスポーンは非介入（別増分）。
  - **G. 検証**: `selfcheck-mineroad-bury.mjs` 新規（生成時配置の決定論 startDive 2 回一致 / 圏外 tick 停止（hp 不変・脱出なし）/ 圏内 HP−1 蓄積 / bury100 種の覚醒後脱出 + 空間化（dug 入り）/ 脱出タイミングの決定論（同操作列 2 回一致）/ 土中死 = 静かに除去（EXP/ドロップ 0）/ 掘り当てアクティブ化 + 非前進 / buried 非 bump・非描画 / 崩落再埋没 / tileType/girlPositions/oreAt/hazardAt 非介入）+ **既存テストの明示的追随（テスト緑化と区別して本エントリを根拠に書き換え）**: debug gate S2（掘削時 bury% 出現の assert）は機構替えに追随（掘り当てで「そこに居た」個体が出る + 決定論へ）。既存 selfcheck 6 本 + debug gate A の VERSION assert は v0.16.0→v0.17.0 の機械追随。fun/funproxy は挙動変化（圧が上がる方向: 全埋没個体が実体化し自力で出てくる）で結果が変わりうるため再計測して honest 記録（数値ノブの単独いじりなし。壊れた場合は BURY_PRESENCE_RATE でなく判断 C の wake range を先に見直す=密度は遭遇率保存の核のため）。既存作 URL 不変・短高 viewport はみ出し 0・本番 47825 非接触（別ポート検証・PID 直 kill）。
- **設計見直し（2026-07-19、対症療法 2 周目ルール発動 = 上位 `~/companion/CLAUDE.md`）**: 判断 A〜G の verbatim 実装 + バッテリー 3 回連続 ALL PASS 後の fun/funproxy 実測で、全体力尽き率 25%→50%、funproxy 最下層到達 20/20→**0/20**（47 手で「進めなくなって終了」= 脱出個体が坑道を塞ぎ bot が bail）。判断 G 先記録の予備手（wake range 8→4）を 1 回適用したが**全 bot 指標が range 8 と同値** = range は効かない。3 度目の数値調整を打たずに原因を実測特定して設計へ遡った:
  1. 裏庭の埋没構成は **28/29 体が bury100**（SPIDER21+WORM7、帯別重み × 配置最終ゲート h<bury/100 が bury100 種を素通しする畳み込みの帰結）。bury100 は圏内 1 tick で必ず即脱出するため、range は「開始時の同時覚醒数」しか絞れず（実測 14→6 体）、自機が盤面を動けば累計脱出数はほぼ全数に達する。
  2. 圧の実体 = 脱出済み個体が bfsStep（坑道網全域の完全 BFS、guard 4000、app.js:1992）で収束してくる**累積戦闘量**。原作の移動 AI は経路探索なしの signum 直線 + 確率ゲート（spec §5）で迷路を解けず、活動 16 マス圏 + 28 マス despawn で常時交戦数も抑えられる — リメイク「賢い追跡 × 少数実体化」前提と原作「大量だが鈍い」前提の衝突が全実体化で露出した。
  3. 根本 = 判断 A「遭遇率保存」が保存した量の取り違え: BURY_PRESENCE_RATE 0.16 は v0.5.0 で「掘りイベント率」（掘った面でしか実体化しない前提）として設計された値で、生成時全実体化に持ち込むと**人口密度**へ意味が変わる（裏庭 15×15 で埋没 29 体 = 空間スポーンと同規模の常備軍）。原作の埋没配置は生成時スポーンの WORM 特例のみ = 疎な脇役（spec §5）。
  - **裁定 = 判断 A を改稿して設計を引き直す**（2 周目の合理化はしない）: ①**役割分離** — bury%（PER_BROKEN_SOIL）を配置ゲートから外し脱出抽選専用へ（原作の役割どおり。配置 = presence × 帯別重み（bury>0 種）のみ。ハッシュ消費は bury 独立レイヤー内の変更で tileType/girlPositions/oreAt/hazardAt の系列には非介入のまま）。②**密度の意味替え再導出** — BURY_PRESENCE_RATE 0.16→**0.05**。「掘りイベント率」→「埋没人口密度」への意味替えに伴う再導出であり難度ノブの単独いじりではない（裏庭で埋没 29→約 9 体 = 原作の疎な埋没規模へ。掘り当てジャンプスケアの頻度は下がるが、原作も掘り当ては稀で主は「勝手に出てくる」= 自力脱出が代替する緊張源）。③判断 B〜F（脱出機構・決定論・wake range 4・崩落再埋没・参照境界）は不変。
  - **既知残差分（本増分では踏み込まない）**: bfsStep 完全追跡 vs 原作 signum 直線 + 活動範囲 16 + despawn 28（台帳④「活動/despawn」）は v0.5.0 以来の手触りの核のため触らない。密度再導出後も実機で過圧なら、この AI/活動範囲の原作化を次増分として台帳から発注する。
- **実装（implementer 委任、2026-07-18〜19、VERSION v0.16.0→v0.17.0）**: 判断 A〜G + 設計見直し裁定を verbatim 実装。app.js: trySpawnBuryMonster 削除、生成時配置（startDive で全固体マス走査 → buried=true / bt:0 / origCol/origRow を持つ個体を配置、G.spawned の bury 用途廃止）、土中 tick（圏内判定 → HP−1 → HP0 は silent splice = EXP/kills/drops 非加算 → bt++ → 脱出抽選）、activateBuriedAt（掘り当て = 旧 trySpawnBuryMonster の演出 + 非前進分岐を流用）、脱出の世界作用は act 掘り抜きと同順（digProgress.delete→dug.add→fallen.delete→markUnstableDug→releaseFluidAt、collect 系は非呼出=採取なし）、resolveCaveins 再埋没（orig/bt を着地マスへ付け替え、space 個体はここで初めて orig を持つ）、monsterAt/render の buried 除外、CONST.BURIED_WAKE_RANGE=4。tiles.js: buryEscapeRoll 新設 = hash3(origCol+5347, origRow+6473, seed+9403+bt×7717)（既存全系列と非衝突、bt≥1 で offset≥17120）、配置最終ゲート（+1019/+643/+6173）撤去=役割分離（空き位相としてコメント記録）、BURY_PRESENCE_RATE 0.16→0.05（意味替え再導出のコメント書き直し）。**実装裁量（記録）**: 脱出演出は SFX 常時 + popup は事前 seen セルのみ + hint なし（当初の seen 時 hint 案は gate C2=女の子発見 hint を上書きして衝突 → hint 枠は自機主語イベント専用の既存運用に整合させ撤去、実装修正 1 回・2 周目なし）。gate S1 は startDive が buried も配置する直接帰結として !m.buried 絞りの追随。実測（裏庭 seed 41027）: 埋没 **17 体 = SPIDER11/WORM3/SLIME3**（裁定前 29 体 SPIDER21/WORM7/SNAKE1）。バッテリー（selfcheck 7 本 + debug 全 gate A〜AF）**3 回連続 ALL PASS**、決定論静的検査（Math.random/Date.now 実呼び 0）PASS。
- **fun/funproxy 4 点比較（v0.16.0 / r8·0.16 / r4·0.16 / r4·0.05·役割分離、honest 記録・追加調整なし）**: fun 全体力尽き率 25%/50%/50%/**50%**、optimal 探索 93%→82%→82%→**95%**・手数 452→270→269→**331**（探索軸はベースライン水準へ回復、終盤力尽き 5/5 は残存。bot は撤退機能を使わない=撤退 0 のため人間難度を過大評価する側の指標）、funproxy 到達深度 median 15/6/6/**3**・行動数 68/47/47/**4001（上限）**。funproxy の張り付きは「bot が bump 1 発で横へ逸れ戦闘を完遂しない方針 × 脱出個体の bfsStep 収束」の測定系限界と解剖（fun 側=戦闘完遂 bot は同盤面で探索 95% まで回復）。**狙い（25% 近辺・最下層回復）には未達のまま出荷**し、残圧の主因 = bfsStep 完全追跡（設計見直しの既知残差分）の原作化は OWNER 実機の手触りで次増分化を判断する。
- **playtester 独立検証（2026-07-19、3 回連続 ALL PASS・fail 0）**: `tests/verify-mineroad-bury.mjs` 新設（selfcheck とは別実装・state 注入 0 件・期待値は盤面ダンプから事前導出したリテラル・全手 page.mouse + elementFromPoint 最前面 assert）。生成時 17 体リテラル一致 + **非描画をピクセル diff=0 で証明**（アクティブ化後 diff=136 で描画復帰）/ 掘り当てアクティブ化 + 非前進 + popup/ヒント + 反撃の SP 台帳一致 / SLIME(bury30) の衰弱（hp15→14）→ 事前導出 escBt=2 ちょうどの脱出（dug 入り・fog→空間 diff=186）+ bury100 の tick1 即時脱出 / 圏外 13 体は全 18 手不変 / buried 非 bump（タップが攻撃でなく掘削になる）/ 決定論 18 スナップショット × fresh 2 回 × 3 run 完全一致 / 既存 5 URL+healthz 200・短高 412×680/730 はみ出し 0・pageerror 0。本番 47825 非接触（47894 自前起動 → PID 直 kill → healthz 200）。**構造的観測（仕様どおりだが記録価値）**: 裏庭の帯構成（bury100 偏重 + escBt≤2）では**土中死は実プレイで発生し得ない**（脱出が先に成立、selfcheck の注入検証のみでカバー）。崩落再埋没も裏庭なだれ 0 で同様。いずれも深いダンジョン（HP 高 × bury 低種・なだれ帯）で初めて実プレイに発現する。
- **code-reviewer（2026-07-19、修正必須なし）**: 判断 verbatim 整合（変異順・「掘りかけタップ中は monstersAct が走らず脱出に先回りされない」構造含む）、hash3 全 15 呼び出しの位相列挙照合（新位相非衝突・純関数のため旧ゲート撤去による他系列シフトなし）、monsterAt 除外の残存読み手（applyGravity/bfsStep）の無害性、テスト実走（selfcheck-bury 20/20 + debug 全 gate + water 28/28）を確認。軽微 2 件: ①debug-mineroad ヘッダーの旧値「Chebyshev 8」残存 → **commit 前に修正済み** ②selfcheck-bury check 10c の hazardAt 同値比較がトートロジー（verify 側の期待値リテラルで suite 全体では担保）→ 次回テスト編集時に golden 値化 = follow-up。観察: 再埋没された bury0 種（BAT）は脱出不能で衰弱死のみ = PER_BROKEN_SOIL 0 の意味どおりで整合。
- **follow-up（次増分候補）**: AI/活動範囲の原作化（bfsStep 完全追跡 → 原作 signum 直線 + 確率ゲート + 活動 16 マス圏 + despawn 28。当初「実機で過圧ならキュー先頭へ」の手触り待ちゲートを置いたが、**OWNER 方針 2026-07-19「手触りより原作準拠を優先」により撤回して発注対象化 = #123 起票済み**）/ ランタイムスポーン（キュー 3 位。埋没人口の補充源として本機構とセットで効く）/ selfcheck-bury 10c の golden 値化 / 土中死・崩落再埋没の実プレイ発現は深ダンジョンの帯構成で自然に出るため状況待ち。
- **進捗**: **完了（2026-07-19）**。翻案判断 先記録 → implementer 委任 → 実測で過圧検出 → wake range 予備手 1 回 → **2 周目ルール発動で設計見直し裁定を先記録 → 役割分離 + 密度再導出の再実装** → playtester 独立検証 3 回 ALL PASS → code-reviewer 修正必須なし → commit。live-from-disk のため commit 時点で本番反映（2026-07-06 裁定ゲート充足、v0.16.0 と同じ扱い）。差分台帳キュー 3 位（ランタイムスポーン）以降は #119 台帳から別セッションで発注。

### v0.18.0（2026-07-19 着手、モンスター AI/活動範囲の原作合わせ＝v0.17.0 既知残差分（残圧の主因）、チケット #123、**着手時 翻案判断 先記録**）

OWNER 方針 2026-07-19「手触りより原作準拠」により v0.17.0 の実機手触り待ちゲートを撤回して発注（#123）。着手時の一次再確認（デコンパイル直読 + `jadx --show-bad-code` での `bj.b`/`ci.a` 再復元）で、**正本 spec §5 の「移動 AI = signum 直線 1 マス接近 + 確率ゲート」が監査 #119 の読み違い**と判明したため、正本訂正を判断 A として先頭に置く。チケット文面の「signum 直線 1 マス接近へ」は正本訂正後の実体（= 8 方向隣接時のみ成立するバンプ攻撃ゲート）として実装する。

- **原作仕様 → web 実装への翻案判断（着手前に記録）**:
  - **A. 正本訂正（spec §5）— 原作モンスターは自機へ接近しない**: `a(bo,P)`（`bk.java:62-74`）の移動先判定 `g()`（`bk.java:157-166`）は `nVar.al(x,y)` を要求し、`al` は「(x,y) が自機セルに一致」（`n.java:1749-1751`、`this.i` は `cm` = 自機、`n.java:24/75`）。つまり **signum 一歩が自機セルに一致する場合＝ Chebyshev 隣接時のみ P% でバンプ攻撃**であり、遠方からの接近には決して使われない。`a(context,bo,P)`（`bk.java:44-57`）は §8 記載どおり隣接 8 マスの GIRL ユニットへの P% 攻撃（ダメージ STR）。移動は**種別の徘徊のみ**: BAT=`bl` 8 方向ランダム飛行 / SLIME=`ce`・SLIME_HALF=`cf` 重力落下優先 + dx∈{-1,0,1}·dy∈{-1,0} ランダム / SNAKE=`cg` 方向持続パトロール（反転・1 段登降） / SPIDER=`ci` dx∈{-1,0,1}·dy=75%:0/25%:−1 + 糸・天井状態機械 / WORM=`cj` 土中クロール、空間では落下 + 横ランダム。ID→AI 対応は `bj.b` の bad-code 再復元で一次確定（BAT=bl, SLIME=ce, GIRL=bt, SNAKE=cg, SLIME_HALF=cf, WORM=cj, SPIDER=ci, SOIL_DRAGON=bp — §0 表と整合）。spec §5 を本エントリと同時に訂正し、訂正後の §5 を本増分の正本とする。
  - **B. 機構替え = bfsStep 全廃 → 種別徘徊 + バンプ確率ゲート**: monsterStep の bfsStep 追跡（坑道網全域 BFS、v0.5.0 以来のリメイク独自）を削除し、原作の 2 層へ: ①**偶発バンプ** — 徘徊/落下の移動先が自機セルなら移動せず攻撃（`bo.java:359-367` verbatim。WORM 含む全種）。②**意図的バンプゲート** — 種別確率・種別順序で 8 方向隣接攻撃を試行: BAT=女80%→自80%（`bl.java:28`）/ SLIME・HALF=女100%→自80%（`ce.java:31`/`cf.java:29`）/ SNAKE・SPIDER=自100%→女50%（`cg.java:24-31`/`ci.java:40-44`）/ WORM=攻撃なし（`cj` に攻撃呼び出しなし = 無害な土の住人）。斜めバンプ・斜め徘徊は corner-cut 規則（`n.c`、両直交隣が塞がりなら不可 = v0.15.0 斜め掘り規則と同型）。女の子攻撃対象は following のみ（前提: 原作の埋没 GIRL ユニットへの攻撃可否は未確認。リメイクの hidden は土中で別レイヤーのため現行どおり対象外）、ダメージは現行式 max(1, STR) 維持（乱数幅式は台帳④既知の別翻案）。ゲート失敗時は徘徊へ落ちる = 隣接しても交戦ロックしない（原作どおり出入りする）。
  - **C. SP-睡眠サイクル（原作の緩急の実体）**: 徘徊移動 1 歩 = SP−1、SP0 で入眠（`bk.java:34-42`）。眠り中は行動なし（攻撃もしない）+ 毎ターン SP回復値ずつ回復、満タンで覚醒（`bk.java:13-28`）、被弾で即覚醒（各 `j()`）。MONSTER テーブルへ sprec を追加（monster.csv SP回復列 verbatim: BAT2/SLIME2/HALF2/SNAKE2/WORM0/SPIDER2）。WORM は SP 消費なし（`cj` に SP コスト呼び出しなし）= 眠らない。眠り個体の描画は実装裁量で記録（原作は専用スプライト）。BAT 天井ぶら下がり・SPIDER 糸（状態 8/9）・SLIME 分裂/合体（`ce.l`/`cf` n≥8）・SNAKE/SPIDER の鳴き SFX は移動 AI の骨格ではないためスコープ外 = 既知残差分として台帳へ。
  - **D. SPD 剰余の原作意味**: 移動への SPD 剰余ゲート（d % SPD ≠ 0 スキップ）は `cj`（WORM、`cj.java:31-33/72-74`）のみ一次確認。bl/ce/cf/cg/ci に SPD 参照なし（br/bs は固定 d%2 だが対象外種）。よって現行の「spd≦1 隔ターン追跡」翻案（追跡前提の cd スロットル）を撤去し、**WORM のみ活動ターンカウンタ tk % SPD(=3) == 0 のターンに徘徊、他 5 種は毎ターン徘徊**（緩急は判断 C の睡眠が担う）へ。
  - **E. 活動範囲 16 / despawn 28 の verbatim 導入（チケット名指しのスケール翻案は「不要」と裁定）**: リメイクの DUNGEONS 寸法（15×15〜80×80）は原作 dungeon_info.csv と**同一**と照合済みで、「リメイクは小マップ」という前提が成立しない（v0.17.0 判断 C の「おおむね原作の半分規模」は誤認だった）。よってスケーリングせず原作値をそのまま: ①**活動** = 自機から両軸 ±16 の箱（`by.java:227-229` verbatim = Chebyshev 16）内の個体のみターン処理（徘徊・攻撃・SP 回復・睡眠・土中 tick すべて）。圏外は完全凍結（tk も進まない）。裏庭 15×15 では常に全員圏内 = 原作の裏庭と同じ。②**despawn** = **x・y 両軸とも** ±28 圏外のとき除去（`by.java:170-172` の逐語 `(x外)&&(y外)`。片軸のみ圏外では消えない、という原作の癖をそのまま採る）。判定は毎ターン全個体（活動圏外・buried 含む。原作 by ループと同じ）、GIRL/SOIL DRAGON 除外はリメイクに該当個体なし（女の子は別システム・SOIL DRAGON 未実装）で空適用。despawn は splice のみで補充なし = ランタイムスポーン（キュー 3 位）導入までは大マップ遠方の人口が漸減する片肺状態と明記（原作は 30% 追加湧きとセット）。③ **BURIED_WAKE_RANGE(4) は撤去し活動箱 16 へ統一** — 原作は活動箱 1 本で `bk.e`（土中 tick）も律速する。v0.17.0 判断 C の range 根拠（半分スケール）は本エントリの寸法照合で崩れた + 同裁定の実測で「range は bot 指標に効かない」ことも確認済み。2 周目ルール整合: これは数値いじり 3 度目ではなく「リメイク独自ゲートの撤去 + 原作単一ゲートへの統一」の設計変更で、根拠（寸法同一の一次照合）を本エントリに記録する。
  - **F. 決定論**: 徘徊方向・各確率ゲートは個体別ロールカウンタ rc（draw ごと +1、活動圏内でのみ進む）による hash3 ストリーム `hash3(spawnCol+P1, spawnRow+P2, seed+P3+rc×P4)` で確定（位相 P1..P4 は既存全系列と非衝突の新値を実装時採番・記録。Math.random/Date.now 実呼び 0 維持）。同一 seed + 同一操作列 → 徘徊・攻撃・睡眠・despawn とも完全再現。
  - **G. 参照境界/非介入**: tileType/girlPositions/oreAt/hazardAt/流体/なだれ/埋没機構（v0.17.0 判断 A〜F）へは非介入。**接地種（SLIME/HALF/SNAKE/SPIDER/WORM）に重力を導入**（下が空間なら徘徊より先に 1 マス落下、落下ターンは攻撃なし = 原作の分岐順。落下先が自機セルなら bo.a 経路で攻撃 = 上から降ってくる）。BAT は重力なし（飛行）。モンスターと流体の相互作用（`bl` の水中ダメージ・出現地形フラグでの移動制限）は未実装のまま既知残差分として台帳へ。moveTo/act/女の子追従/崩落再埋没（v0.17.0 E)は不変。
  - **H. 検証**: `selfcheck-mineroad-ai.mjs` 新規（活動箱 16: 圏外個体の座標/SP/tk/hp 完全凍結 / despawn AND 条件: 片軸 29+ 離れのみでは残存・両軸で除去（孤独な山 80×80 相当の盤で構成）/ 徘徊の決定論 fresh 2 回一致 / バンプゲート確率の決定論再現 / SP0 入眠→回復→満タン覚醒→被弾即覚醒 / WORM tk%3 と攻撃なし / 接地種の重力落下・BAT 非落下 / 斜め corner-cut / 女の子攻撃の種別順序 / buried tick が活動箱 16 で律速されること）+ 既存テストの明示的追随（本エントリを根拠に記録して書き換え: debug gate の追跡前提 assert、selfcheck-bury の BURIED_WAKE_RANGE 参照、VERSION assert の v0.18.0 機械追随）。fun/funproxy は再計測して honest 記録（bfsStep 撤去 + 攻撃確率ゲートで圧が大きく下がる方向。狙い 25% 近辺へ戻るかを記録するのみで、逸脱しても原作値（16/28/確率/密度）の単独調整はしない — 直すなら測定系かランタイムスポーン（キュー 3 位）側で次増分として判断）。playtester 独立実タップ検証 + code-reviewer。既存作 URL 不変・短高 viewport はみ出し 0・本番 47825 非接触。
- **実装（implementer 委任、2026-07-19、VERSION v0.17.0→v0.18.0）**: 判断 A〜H を verbatim 実装。app.js: monstersAct を「despawn（両軸 AND ±28、毎ターン全個体・buried 含む・splice のみ）→活動箱（Chebyshev >16 は tk/rc 含め完全凍結）→tk++→monsterStep」へ書き換え、monsterStep 全面書き換え（buried→sleeping→WORM tk%SPD→重力（BAT 以外、落下ターン攻撃なし・落下先自機セルは偶発バンプ）→意図的バンプゲート→種別徘徊）、BUMP_GATE/DIRS8/aiRoll/monsterDiagPassable/tryBumpGate/spendMonsterSp/tryWanderStep/monsterWander 新設、bfsStep・isAdjacent・spd≦1 cd スロットル削除、buriedTick の wake 距離ゲート撤去（活動箱へ統一）、CONST: BURIED_WAKE_RANGE 撤去・MONSTER_ACTIVE_RANGE 16 / MONSTER_DESPAWN_RANGE 28 追加、attackMonster に被弾即覚醒、drawMonster に眠り「z」淡色描画。tiles.js: MONSTER へ sprec 追加（SP回復列 verbatim: BAT2/SLIME2/HALF2/SNAKE2/WORM0/SPIDER2）、monsterAiRoll 新設 = hash3(spawnCol+6841, spawnRow+7351, seed+11939+rc×8117)（第1オフセットは既存最近値と 1494 差 > cols 最大 80、第2は 350/568 差 > rows 最大 99 = 盤面内座標で他系列と同一入力に到達しえない。既存全系列コメントに列挙記録）。**実装裁量（記録）**: 眠り描画 = 右上淡色「z」/ WORM の tk%SPD は行動関数冒頭で落下も律速（cj.java:31-33 の早期ゲート位置）/ SP 消費は徘徊移動成立時のみ（落下・反転・攻撃は非消費）/ SNAKE パトロール試行順 = 平行→登→降→反転、初期方向は rc ロール / 被弾覚醒は sleeping 解除のみ SP 非回復 / ゲート試行は対象隣接時のみ 1 ロール・確率 100 でもロール消費・女の子複数隣接時のみ +1 ロール / 徘徊移動先の女の子セルは非ブロック（偶発バンプは自機セルのみ = bo.java:359-367 逐語）/ spawnCol/spawnRow をロールストリーム恒久 ID として rebury で動く origCol/origRow と分離。バッテリー（selfcheck 8 本（ai 新規 18 項目含む）+ debug 全 gate A〜AF）**3 回連続 ALL PASS**、決定論静的検査（Math.random/Date.now 実呼び 0）PASS。既存テスト追随: selfcheck-bury §2 を活動箱検証（孤独な山 80×80 盤で圏外構成）へ、debug S3/S4/S5 を徘徊揺らぎ排除の固体化構成へ（根拠コメント明記、緑化目的ではない）。
- **fun/funproxy 実測（v0.17.0 比、honest 記録・調整なし）**: fun 力尽き率 50%（同値）・optimal 探索 95%（同値）・手数 324（331）、funproxy 到達深度 median 3（同値）・行動数 4001 上限（同値）= **bot 指標は機構替え前後でほぼ不変**。v0.17.0 の「残圧の主因 = bfsStep 追跡」仮説は bot 指標上は支持されなかった（fun bot は戦闘完遂方針で自ら隣接し続けるため追跡有無が効かず、funproxy は bump で逸れる方針の測定系限界 = v0.17.0 解剖どおり）。「モンスターが寄ってこない」変化は人間の手触りに出る性質のため、判断 H どおり原作値は触らず出荷し OWNER 実機の体感を待つ。
- **playtester 独立検証（2026-07-19、verify-mineroad-ai.mjs 新設 3 回 + 追随後の verify 2 本 × 3 回 すべて ALL PASS・fail 0）**: 隣接 100% 種の毎ターン被弾（SP 台帳一致）と離脱後の被弾 0 / SPIDER 25 ターン徘徊トラックのリテラル完全一致（隣接 0 回・反転 3 回 = 追跡なしを実測）/ fresh 3 run × 32 スナップショット完全一致 / SP5→0 入眠→sprec2 回復→覚醒 + 眠り「z」ピクセル diff=11（対照 0）/ 一斉脱出 dug 16 マスリテラル一致 + 距離 8 個体の tick = 活動箱 16 の実効を距離で証明 / 全 URL+healthz 200・短高はみ出し 0・pageerror 0。**観察（仕様どおりだが記録価値）**: ①裏庭は活動箱が常に全域を覆い埋没 17 体中 15 体が初回行動で一斉脱出 =「近づくと動き出す」手触りは裏庭では消滅（原作 verbatim の帰結。深ダンジョンで初めて段階覚醒が発現）②一斉脱出が遠方の湧水を解放しうる ③裏庭の閉鎖ポケットでは脱出個体が gridlock で不動（徘徊が見えるのは連結空間のみ）④**モンスターは徘徊で地表 row0 へ上がれる**（原作の地表徘徊可否は一次未照合 → follow-up）。前任 playtester がリミット断で離脱したため verify-mineroad-bury.mjs の v0.18.0 追随（wake4 前提 8 件の陳腐化検出→bury 固有検証（非描画ピクセル・bt カウンタ・掘り当て非攻撃・撃破 end-to-end）へ再構成 + 重複は ai 側へ委譲）は後任 playtester が実施。
- **code-reviewer（2026-07-19、修正必須なし）**: 判断 A〜H/spec §5 訂正版との verbatim 整合（分岐順・BUMP_GATE 確率順序・despawn 逐語 AND・活動箱凍結の網羅・corner-cut 同型）、monsterAiRoll 位相の非衝突（オフセット差 > 盤面最大寸法の数学的成立を実測確認）、撤去残渣 grep 0 件、G.monsters 非永続（毎 startDive 再生成）でセーブ復元の新フィールド欠落なし、selfcheck-ai 18/18 + debug 全 gate 実走 PASS を確認。軽微 1 件: 崩落再埋没時に sleeping 非リセット（描画リークなし・状態衛生のみ）→ **不適用と裁定**: 原作の睡眠フラグ（bo.i）は埋没で消えず持続するため非リセットがむしろ忠実。記録のみ。
- **follow-up（次増分候補）**: ランタイムスポーン（キュー 3 位。despawn 片肺（補充なし人口漸減）の解消とセットで効く）/ モンスターの地表 row0 徘徊可否の原作照合（playtester 観察④）/ 種別特殊挙動（BAT 天井・SPIDER 糸・SLIME 分裂合体・水中ダメージ・出現地形フラグ移動制限）は台帳④既知残差分 / selfcheck-bury 10c golden 値化（v0.17.0 継続）。
- **進捗**: **完了（2026-07-19）**。一次再確認で spec §5 の読み違いを発見・正本訂正 → 翻案判断 A〜H 先記録 → implementer 委任（verbatim + バッテリー 3 回 ALL PASS）→ playtester 独立検証（前任リミット断→後任が verify-bury 追随、verify 2 本 × 3 回 ALL PASS）→ code-reviewer 修正必須なし → commit。live-from-disk のため commit 時点で本番反映（2026-07-06 裁定ゲート充足、v0.17.0 と同じ扱い）。
