...
tokens の生テキストでなくここに揃える) は 3 段フォールバック (チケット #80):
  1. wordlists/confusions.json の音韻混同グループ (聞き分け練習の核、最優先)
  2. wordlists/categories.json の同文法カテゴリ (所属する全カテゴリの合併、正解と同じ
     文法スロットに入り得る語 — 「文法だけで解ける」誤答肢を排除する)
  3. weak_forms+common2000 全プールからランダム (最後の保険。カテゴリ未所属の語 —
     content word が空欄になった場合など — はここに落ちる)
正解位置はシャッフル。

confusions.json の形式: {"groups": [[word, word, ...], ...]} — 内側配列 1 つが
「聞き分けにくい語の集合」(大小無視でマッチ)。
categories.json の形式: {"categories": {name: [word, ...], ...}} — 1 語が複数カテゴリに
属してよく、その場合の誤答肢候補は所属カテゴリの合併。
...
`--refresh-blanks <episode_id|all>` で、既存クリップ (mp4 は変更しない) の
tokens/blanks/feature_tags を build_clip_fields の再実行で UPDATE できる (再エンコードなしの
バックフィル用、wordlists の誤答肢規則変更を既存クリップに反映する)。tokens は text.split()
で決定的なので不変、空欄 idx/answer も wordlists の対象語が同一なら _pick_blanks の決定性
により不変 — choices だけが改善される。
"""
import argparse
import json
import os
...
def _load_categories(path):
    """categories.json を読み、{word: 同カテゴリ語の合併 set (自分自身を除く)} を返す。
    1 語が複数カテゴリに属する場合は所属する全カテゴリの合併になる。"""
    if not path.is_file():
        return {}
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
    ...
    for words in categories.values():
        ...
        for w in words:
            mapping.setdefault(w, set()).update(x for x in words if x != w)
    return mapping
...
def _build_choices(answer, other_answers, confusion_groups, category_map, fallback_pool, rng):
    """誤答肢 3 語を 3 段フォールバックで選ぶ: 音韻混同グループ → 同文法カテゴリ (合併、
    ランダム順) → 全プールランダム (最後の保険)。カテゴリ未所属の語 (content word が空欄に
    なった場合など) は従来どおり全プールランダムに落ちる。"""
    group = None
    ...
    for c in candidates:
        ...
        if len(choices) >= NUM_CHOICES - 1:
            break
    if len(choices) < NUM_CHOICES - 1:
        category_candidates = sorted(category_map.get(answer, ()))  # sort → shuffle で決定的
...
def build_clip_fields(text, weak_forms, common2000, confusion_groups, category_map,
                      fallback_pool, rng):
    ...
    for pos, idx in enumerate(chosen):
        ...
        choices = _build_choices(answer, other, confusion_groups, category_map,
                                 fallback_pool, rng)
...
def _load_all_wordlists():
    """wordlists/ 一式を読み (weak_forms, common2000, confusion_groups, category_map,
    fallback_pool) のタプルで返す (process_all/rebuild/refresh の共通前処理)。"""
    ...
    category_map = _load_categories(common.WORDLISTS_DIR / "categories.json")
    fallback_pool = sorted(weak_forms | common2000)
    return weak_forms, common2000, confusion_groups, category_map, fallback_pool
...
def process_episode(conn, ep, weak_forms, common2000, confusion_groups, category_map,
                    fallback_pool):
    ...
    for s in candidates:
        ...
        fields = build_clip_fields(s["text"], weak_forms, common2000, confusion_groups,
                                    category_map, fallback_pool, rng)
        if fields is None:
            skipped_no_blank += 1
            continue
...
def rebuild_episode(episode_id, db_path=None):
    ...
    weak_forms, common2000, confusion_groups, category_map, fallback_pool = _load_all_wordlists()
    made, skipped, err = process_episode(conn, ep, weak_forms, common2000,
                                          confusion_groups, category_map, fallback_pool)
    if err:
...
def refresh_blanks(episode_id, db_path=None):
    """再エンコードなしで既存クリップの tokens/blanks/feature_tags だけ埋め直す
    (mp4 は触らない、UPDATE のみ — fill_translations と同じバックフィル方式)。

    clips.text から build_clip_fields を再実行する。tokens は text.split() で決定的なので
    既存値と同一になり、空欄 idx/answer も wordlists の対象語が同一なら _pick_blanks の
    決定性により不変 — 実質 choices だけが新しい誤答肢規則 (confusion group → 同文法
    カテゴリ → 全プールランダム) で改善される。rng はクリップ単位で決定的
    (random.Random(clip_id)、process_episode と同じ)。

    安全確認として再計算 tokens が既存値と一致しない行、および build_clip_fields が
    None を返す行 (理論上ないはず) は UPDATE せず skip してログに残す。"""
    conn = common.open_db(db_path)
    ep = conn.execute("SELECT id FROM episodes WHERE id=?", (episode_id,)).fetchone()
    if ep is None:
        raise ValueError("episode not found: %s" % episode_id)
    weak_forms, common2000, confusion_groups, category_map, fallback_pool = _load_all_wordlists()
    rows = conn.execute(
        "SELECT id, text, tokens FROM clips WHERE episode_id=?", (episode_id,)).fetchall()
    updated = skipped = 0
    for row in rows:
        rng = random.Random(row["id"])
        fields = build_clip_fields(row["text"], weak_forms, common2000, confusion_groups,
                                    category_map, fallback_pool, rng)
        if fields is None:
            common.log("refresh-blanks skip %s: build_clip_fields returned None" % row["id"])
            skipped += 1
            continue
        tokens, blanks, feature_tags = fields
        if tokens != json.loads(row["tokens"]):
            common.log("refresh-blanks skip %s: tokens mismatch (existing row kept)" % row["id"])
            skipped += 1
            continue
        conn.execute(
            "UPDATE clips SET tokens=?, blanks=?, feature_tags=? WHERE id=?",
            (json.dumps(tokens, ensure_ascii=False), json.dumps(blanks, ensure_ascii=False),
             json.dumps(feature_tags, ensure_ascii=False), row["id"]))
        updated += 1
    conn.commit()
    common.log("refresh-blanks %s: updated=%d skipped=%d (total clips=%d)" %
               (episode_id, updated, skipped, len(rows)))
    return updated, skipped


def refresh_blanks_all(db_path=None):
    """clips を持つ全 episode に refresh_blanks を回す (`--refresh-blanks all`)。"""
    conn = common.open_db(db_path)
    episode_ids = [row["episode_id"] for row in conn.execute(
        "SELECT DISTINCT episode_id FROM clips ORDER BY episode_id")]
    conn.close()
    total_updated = total_skipped = 0
    for episode_id in episode_ids:
        updated, skipped = refresh_blanks(episode_id, db_path=db_path)
        total_updated += updated
        total_skipped += skipped
    common.log("refresh-blanks all: episodes=%d updated=%d skipped=%d" %
               (len(episode_ids), total_updated, total_skipped))
...
def main(argv=None):
    ...
    parser.add_argument("--refresh-blanks", default=None, metavar="EPISODE_ID|all",
                         help="指定 episode (または all で全話) の既存クリップの "
                              "tokens/blanks/feature_tags を埋め直す (mp4 は変更しない、"
                              "UPDATE のみ — wordlists の誤答肢規則変更の反映用)")
    ...
    elif args.refresh_blanks:
        if args.refresh_blanks == "all":
            refresh_blanks_all(db_path=args.db)
        else:
            refresh_blanks(args.refresh_blanks, db_path=args.db)
    else:
